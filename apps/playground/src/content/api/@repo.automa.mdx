...
`@repo/automa` is the interactive WebGL2 workbench half of the cellular automaton ecosystem. It wraps the pure simulation engine ([`@repo/automa-engine`](/discoveries/automa-engine/)) in an interactive R3F / Glaze canvas with GPGPU stepping, brush painting, creature stamp patterns, and a rule selector.
