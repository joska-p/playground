const drawFactorWaves = {
    ...
    draw: (ctx, data, params, layout) => {
        const { lineWidth = 1.5, alpha = 0.65, amplitudeScale = 0.4, saturation = 85, lightness = 55 } = params;
        const { maxVal, valueScale, offsetX, offsetY } = layout;
        if (maxVal <= 0)
            return;
        const maxAmplitude = ctx.canvas.height * amplitudeScale;
        data.forEach((p) => {
