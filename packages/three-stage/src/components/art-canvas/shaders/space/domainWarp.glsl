// Standard hash function to generate procedural randomness
float hash2d(vec2 p) {
...
float valueNoise(vec2 p) {
  vec2 i = floor(p);
  vec2 f = fract(p);
  f = f * f * (3.0 - 2.0 * f); // Smooth interpolation (smoothstep)
...
// Distorts the coordinate space fluidly over time
