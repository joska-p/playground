...
function isPass(key: CheckKey, value: GlazeProof[keyof GlazeProof] | undefined): boolean {
  ...
  if (key === 'programCpu') {
    ...
    return !!program?.frame36 && program.frame20.ok && program.frame36.ok && program.frame36.present;
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {CHECKS.map(({ key, label }) => {
        ...
        const status = value === undefined ? 'pending' : isPass(key, value) ? 'pass' : 'fail';
        const className =
          status === 'pending'
        ...
        return (
          <span key={key} className={`rounded px-2 py-0.5 font-mono text-[10px] ${className}`}>
            {label} · {status}
          </span>
        );
      })}
