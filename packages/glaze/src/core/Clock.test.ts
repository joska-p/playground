...
import { advanceFree, advanceLooping, advanceOnce, advancePingPong, createClock } from './Clock';
import {
...
describe('advancePingPong', () => {
    it('keeps moving forward below the bound', () => {
        const { time, direction } = advancePingPong(
            second(0),
            createNonNegativeSeconds(0.4),
            createDurationSeconds(1),
            1
        );

        expect(time).toBeCloseTo(0.4, 12);
    ...
    it('never escapes the bounds, even for huge deltas', () => {
        const { time } = advancePingPong(
            second(0),
            createNonNegativeSeconds(10),
            createDurationSeconds(1),
            1
        );
...
describe('advanceLooping', () => {
    it('wraps forward past the bound', () => {
        expect(advanceLooping(second(0.8), second(0.7), createDurationSeconds(1))).toBeCloseTo(
            0.5,
            12
        );
    });
    ...
    it('wraps backwards into the tail of the timeline', () => {
        expect(advanceLooping(second(0), second(-0.25), createDurationSeconds(1))).toBeCloseTo(
            0.75,
            12
        );
    });
...
describe('clock config union', () => {
    ...
    it('accepts non-negative frame deltas in update', () => {
        expectTypeOf<Clock['update']>().parameter(0).toEqualTypeOf<NonNegativeSeconds>();
...
describe('clock behavior', () => {
    it('free-runs by default with no duration and zero progress', () => {
        ...
        clock.update(createNonNegativeSeconds(1)).update(createNonNegativeSeconds(2));
    ...
    it('loops by default when timed', () => {
        ...
        clock.update(createNonNegativeSeconds(1.5));
    ...
    it('stops playing once a one-shot reaches its end', () => {
        ...
        clock.update(createNonNegativeSeconds(0.6));
        clock.update(createNonNegativeSeconds(10));
    ...
    it('ping-pongs between both bounds', () => {
        ...
        const times = [0, 0.4, 0.4, 0.4, 0.4].map(
            () => clock.update(createNonNegativeSeconds(0.4)).time
        );
    ...
    it('ignores deltas while paused and reports a zero deltaTime', () => {
        ...
        clock.update(createNonNegativeSeconds(1));
        clock.pause();
        clock.update(createNonNegativeSeconds(1));
    ...
    it('does not start before play when autoStart is false', () => {
        ...
        clock.play().update(createNonNegativeSeconds(1));
    ...
    it('restarts from zero with direction reset', () => {
        ...
        clock.seek(second(1));
        clock.reset().update(createNonNegativeSeconds(0.25));
