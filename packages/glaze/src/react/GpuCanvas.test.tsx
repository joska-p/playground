...
function createFakeGl() {
  ...
  const gl = {
    ...
    attachShader: vi.fn(),
    linkProgram: vi.fn(),
    getProgramParameter: vi.fn((_program: unknown, pname: number) => (pname === ACTIVE_UNIFORMS ? 2 : true)),
    getActiveUniform: vi.fn((_program: unknown, index: number) =>
      index < 2 ? { name: ['u_time', 'u_custom'][index], type: FLOAT, size: 1 } : null
    ),
...
beforeEach(() => {
  fake = createFakeGl();
  vi.stubGlobal('WebGLTexture', // eslint-disable-next-line @typescript-eslint/no-extraneous-class
    class WebGLTexture {});
  vi.spyOn(HTMLCanvasElement.prototype, 'getContext').mockReturnValue(fake.gl as unknown as WebGL2RenderingContext);
});
...
describe('GpuCanvas', () => {
  it('compiles the fragment shader, drives it every frame, and applies standard + custom uniforms', () => {
    stubRaf();
    const uniforms = (ctx: { time: number }): Record<string, UniformValue> => ({ u_custom: ctx.time });
    ...
    act(() => {
      root.render(
        <GpuCanvas fragmentShader="void main() { out_color = vec4(1.0); }" uniforms={uniforms} onDoor={(door) => doors.push(door)} />
      );
    });
    ...
    const uniformValues = (name: string): number[] =>
      fake.uniform1f.mock.calls.filter((call) => (call[0] as { name: string }).name === name).map((call) => call[1] as number);
  ...
  it('recompiles when the fragment shader changes and frees programs on change and unmount', () => {
    ...
    act(() => {
      root.render(<GpuCanvas fragmentShader="void main() { out_color = vec4(1.0); }" />);
    });
    expect(fake.gl.createProgram).toHaveBeenCalledTimes(1);

    act(() => {
      root.render(<GpuCanvas fragmentShader="void main() { out_color = vec4(0.5); }" />);
    });
