 ...
 * action cycles: `pointerdown` -> `onStart`, `pointermove` -> `onMove`,
 * `pointerup` / `pointercancel` -> `onEnd`, `wheel` -> `onZoom`.
 *
 * Handlers replace the built-in gestures: providing `onStart` or `onMove`
 * turns the default pan off (you own the drag cycle — drive the camera with
 * `event.cameraControls` if you want it to pan), and providing `onZoom` turns
 * the default zoom off. `onEnd` and `onContextMenu` are delivered in addition
 * to the built-ins, so captured state (like an active drag) is always
 * released. `pan` / `zoom` configure the built-in gestures; `false` turns one
 * off, an options object configures it, and omitting it keeps the default
 * behavior.
 *
...
/**
 * Adapts the consumer `CanvasInteractions` config into the pipeline. Custom
 * lifecycle handlers are wrapped to receive a live surface and replace the
 * matching built-in gesture: `onStart` / `onMove` suppress pan, `onZoom`
 * suppresses zoom. `onEnd` / `onContextMenu` run alongside the built-ins.
 */
