...
describe('createInputStore', () => {
  it('tracks the pointer relative to the attached target', () => {
    ...
    store.attach(target);
    target.dispatchEvent(new PointerEvent('pointermove', { clientX: 120, clientY: 80, bubbles: true }));
  ...
  it('converts the pointer to world position through the camera', () => {
    ...
    const store = createInputStore();
    store.attach(target);
    target.dispatchEvent(new PointerEvent('pointermove', { clientX: 20, clientY: 30, bubbles: true }));
    expect(store.getPointerWorldPos({ x: 0, y: 0, zoom: 1 })).toEqual({ x: 20, y: 30 });
    expect(store.getPointerWorldPos({ x: 10, y: 10, zoom: 2 })).toEqual({ x: 5, y: 10 });
    store.detach();
  });
