...
describe('shape uniform builders', () => {
  ...
  it('circle uniforms pass stroke through', () => {
    const uniforms = circleUniforms({ x: 0, y: 0 }, 2, { stroke: '#00ff00', lineWidth: 3 });
  ...
  it('line uniforms prefer stroke over fill over black', () => {
    expect(lineUniforms({ x: 0, y: 0 }, { x: 5, y: 5 }, { stroke: '#ff0000', lineWidth: 4 })['u_color']).toEqual([1, 0, 0, 1]);
    expect(lineUniforms({ x: 0, y: 0 }, { x: 5, y: 5 }, { fill: '#00ff00' })['u_color']).toEqual([0, 1, 0, 1]);
    expect(lineUniforms({ x: 0, y: 0 }, { x: 5, y: 5 }, {})['u_color']).toEqual([0, 0, 0, 1]);
  });

  it('line uniforms carry endpoints and width', () => {
    const uniforms = lineUniforms({ x: 1, y: 2 }, { x: 3, y: 4 }, { stroke: '#ffffff' });
  ...
  it('text uniforms position the baseline and tint the fill', () => {
    ...
    const uniforms = textUniforms({ x: 100, y: 50 }, 60, 40, 28, texture, { fill: '#ff0000' });
