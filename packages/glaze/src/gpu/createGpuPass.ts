...
function createGpuPassTargets(
  ...
  const createTarget = (width: number, height: number): [WebGLTexture, WebGLFramebuffer] => {
    ...
    if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) !== gl.FRAMEBUFFER_COMPLETE) {
      ...
      throw new Error(`Glaze: GpuPass framebuffer incomplete (${String(width)}x${String(height)})`);
    }
  ...
  return {
    ...
    getWriteTexture(): WebGLTexture {
      ...
      if (!texture) throw new Error('Glaze: GpuPass write target not initialized');
      return texture;
    },
...
export function createGpuPass(
  ...
  const activeProgram = (): Program => {
    ...
    if (!program) throw new Error(`Glaze: GpuPass program "${activeName ?? 'default'}" not found`);
    return program;
  };
  ...
  return {
    ...
    useProgram(name: string): void {
      if (!programs.has(name)) throw new Error(`Glaze: GpuPass program "${name}" not found`);
      activeName = name;
    },
