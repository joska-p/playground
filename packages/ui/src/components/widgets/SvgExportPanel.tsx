/** TEMPORARY dev tool — mounts a floating panel that rasterizes live `<svg>` children to PNG/WebP. */
...
export function SvgExportPanel({
    ...
    async function handleExport() {
        ...
        try {
            ...
            if (format === 'webp' && blob.type !== 'image/webp') {
                ...
                setStatus(`Saved ${filename}.${blob.type === 'image/webp' ? 'webp' : 'png'}`);
            }
            downloadBlob(blob, `${filename}.${blob.type === 'image/webp' ? 'webp' : 'png'}`);
        } catch (err) {
            console.error(err);
    ...
    return (
        <div>
            ...
            >
                <div style={{ fontWeight: 600, opacity: 0.9 }}>SVG Export (temp)</div>
                ...
                {status && <div style={{ opacity: 0.8, maxWidth: 220 }}>{status}</div>}
            </div>
        </div>
    );
}
...
/**
 * Rasterizes each live `<svg>` at the target resolution, then composites them onto one canvas in
 * DOM order.
 */
