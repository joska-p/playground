...
2. **Create WorkerPool instance** in `image-pipeline.ts` (or new `pipeline-pool.ts`):
   ...
   const pool = new WorkerPool<RunConfig, ImageData[]>({
     ...
     workerFactory: () => new Worker(new URL('./pipeline-worker', import.meta.url), { type: 'module' }),
     serialize: (task) => ({
       message: { sourceImageData: task.sourceImageData, steps: task.steps, maximumPixels: task.maximumPixels },
