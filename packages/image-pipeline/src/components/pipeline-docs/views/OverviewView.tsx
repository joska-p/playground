  ...
  return (
    <div className="space-y-8">
      ...
      <div className="border-border border-t-utility-2 rounded-lg border border-t-2 p-5">
        ...
        <p className="text-muted-foreground mb-3 text-sm">
          The facade provides a pre-built catalog of all manipulations with UI
          metadata, filterable by access type.
        </p>
        <CodeBlock
          code={`import { imagePipeline } from "@repo/image-pipeline";\n\n// All manipulations\nimagePipeline.manipulations;\n\n// Filter by access type\nconst pixel = imagePipeline.getManipulationsByAccess("pixel");\nconst neighborhood = imagePipeline.getManipulationsByAccess("neighborhood");`}
        />
      </div>
      ...
      <div className="border-border border-t-utility-6 rounded-lg border border-t-2 p-5">
        ...
        <p className="text-muted-foreground mb-3 text-sm">
          Terminate workers and clear the job queue. Call on app teardown.
          Idempotent.
        </p>
        <CodeBlock
          code={`import { imagePipeline } from "@repo/image-pipeline";\n\nfunction App() {\n  useEffect(() => () => imagePipeline.teardown(), []);\n  // ...\n}`}
        />
      </div>
