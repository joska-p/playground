...
/** RGBA with each channel normalized to `[0, 1]`. */
...
function parseRgb(color: string): RGBA | null {
  ...
  const a = parts[3] === undefined ? 1 : parts[3].endsWith('%') ? clamp01(parseFloat(parts[3]) / 100) : clamp01(parseFloat(parts[3]));
  return { r: parseChannel(parts[0]), g: parseChannel(parts[1]), b: parseChannel(parts[2]), a };
}
...
function parseHsl(color: string): RGBA | null {
  ...
  const p = 2 * l - q;
  return { r: hueToRgb(p, q, hue + 1 / 3), g: hueToRgb(p, q, hue), b: hueToRgb(p, q, hue - 1 / 3), a };
}
...
/**
 * Parse a CSS color into normalized RGBA. Supports hex, `rgb()`, `rgba()`,
 * `hsl()`, `hsla()`, and a small set of named colors; anything else falls
 * back to the browser's own parser when a DOM is available. Unparseable input
 * yields a loud magenta so the mistake is visible rather than silently black.
 */
