...
export function createEngine(config: EngineConfig): Engine {
  ...
  const driver = config.driver ?? (config.driverKind === 'gpu' ? createGpuDriver(canvas) : createCpuDriver(canvas));
  ...
  function resize(): void {
    ...
    const height = resizeStrategy === 'window' ? window.innerHeight : canvas.clientHeight;
