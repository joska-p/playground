 ...
 * per-pixel deltas then iterate in double-single (DS) on the GPU.
 *
 * PRECISION LIMITS (do not exceed silently):
 * - float64 gives ~15-16 significant digits, so centers are only trustworthy to
 *   ~1e15–1e16 zoom. For true 1e100 zoom, swap the float64 math here for a BigInt /
 ...
 *
 * Storage layout (important for the DS shader):
 *   data[0]   = X₀ = 0
 *   data[1]   = X₁
 *   …
 *   data[k]   = Xₖ          (last non-escaped or the escaping value)
 *
 * If the reference escapes while computing X_{e+1} from X_e we still write the
 * (large) escaped value into the next slot and set
 *   referenceIterations = e + 2
 * so the shader’s “i+1 < referenceIterations” test succeeds for the escaping
...
/**
 * Compute a second reference a small distance away from the primary.
 * The offset is ~2–3 pixels in complex plane so it is still useful
 * for the current view but statistically less likely to hit the same
