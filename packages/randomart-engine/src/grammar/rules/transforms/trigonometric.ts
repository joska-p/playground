...
export const sinRule = {
  ...
  category: 'structural',
  evaluate: (args) => Math.sin(Math.PI * (args[0]?.() ?? 0)),
  toMathString: (args) => `sin(π · ${args[0] ?? '0.0'})`,
  toGLSL: (args) => `sin(3.1415926535 * ${args[0] ?? '0.0'})`,
...
export const cosRule = {
  ...
  weight: 1.0,
  category: 'structural',
  evaluate: (args) => Math.cos(Math.PI * (args[0]?.() ?? 0)),
  toMathString: (args) => `cos(π · ${args[0] ?? '0.0'})`,
  toGLSL: (args) => `cos(3.1415926535 * ${args[0] ?? '0.0'})`, // Higher precision PI
