...
import { create3DLayout, simulateForces } from '../utils/force3d';
...
function GraphVizContent() {
  ...
  const [layout, setLayout] = useState<ReturnType<typeof create3DLayout> | null>(
    null
  );

  // Initialize and simulate layout
  useEffect(() => {
    if (!data) return;
    ...
    let currentLayout = create3DLayout(nodeSlice, linkSlice);

    // Run simulation in chunks to avoid blocking
    const chunks = 5;
    const iterationsPerChunk = 50;

    const runChunk = (chunkIdx: number) => {
      if (chunkIdx >= chunks) {
      ...
      currentLayout = {
        ...currentLayout,
        nodes: simulateForces(currentLayout.nodes, currentLayout.links, iterationsPerChunk),
      };

      setTimeout(() => runChunk(chunkIdx + 1), 0);
    };
    ...
    return (
      <div className="flex h-screen items-center justify-center">
        <div className="rounded-lg border border-red-500 bg-red-50 p-6">
          <h2 className="mb-2 text-lg font-bold text-red-900">Error Loading Graph</h2>
        ...
        <div className="rounded-lg bg-black/50 px-4 py-2 text-sm text-white backdrop-blur">
          ...
          {hoveredNode && (
            <p className="text-green-300">
              Hover:{' '}
              {layout.nodes.find((n) => n.id === hoveredNode)?.label}
            </p>
          )}
        </div>
      </div>
