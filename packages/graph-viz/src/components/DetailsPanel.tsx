...
const DetailsPanel: FC<DetailsPanelProps> = ({ nodes, links }) => {
      ...
      <Card variant="secondary" className="w-80 backdrop-blur-md bg-secondary/80 border-secondary/30">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              <span className="text-xs text-muted-foreground">In-degree</span>
              <p className="text-sm font-mono font-semibold">{selectedNode.inDegree}</p>
            </div>
            <div className="space-y-1">
              <span className="text-xs text-muted-foreground">Out-degree</span>
              <p className="text-sm font-mono font-semibold">{selectedNode.outDegree}</p>
            </div>
          </div>
          ...
          {(connections.incoming.length > 0 || connections.outgoing.length > 0) && (
            <div className="space-y-2">
              ...
              <div className="max-h-40 space-y-1 overflow-y-auto pr-1">
                ...
                {(connections.incoming.length + connections.outgoing.length) > 20 && (
                  <p className="text-xs text-muted-foreground italic px-2">
                    +{(connections.incoming.length + connections.outgoing.length) - 20} more
                  </p>
                )}
              </div>
            </div>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center">
            <p className="text-2xl font-bold font-mono text-accent">{stats.nodeCount.toLocaleString()}</p>
            <p className="text-xs text-muted-foreground">Nodes</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold font-mono text-primary">{stats.edgeCount.toLocaleString()}</p>
            <p className="text-xs text-muted-foreground">Edges</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold font-mono text-secondary">{stats.communityCount}</p>
            <p className="text-xs text-muted-foreground">Groups</p>
          </div>
        </div>

        <div className="space-y-2">
          <span className="text-xs text-muted-foreground">File Types</span>
          <div className="space-y-1">
