...
export function useGraphSimulation() {
  ...
  useEffect(() => {
    ...
    if (filterFT) {
      const keep = new Set(nodes.filter((n) => n.ft === filterFT).map((n) => n.id));
    ...
    const validLinks = links.filter((l) => nodeMap.has(l.s) && nodeMap.has(l.t));
    ...
    const nodeSel = nodeLayer
      ...
      .call(
        d3
          ...
          .on("start", (e: d3.D3DragEvent<SVGCircleElement, SimNode, SimNode>, d) => {
            if (!e.active) sim.alphaTarget(0.3).restart();
            d.fx = d.x ?? null;
            d.fy = d.y ?? null;
          })
          .on("drag", (e: d3.D3DragEvent<SVGCircleElement, SimNode, SimNode>, d) => {
            d.fx = e.x;
            d.fy = e.y;
          })
          .on("end", (e: d3.D3DragEvent<SVGCircleElement, SimNode, SimNode>, d) => {
