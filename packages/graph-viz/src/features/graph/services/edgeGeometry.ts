import { BufferGeometry, Float32BufferAttribute } from 'three';
import { highlightedEdge, relationPalette } from '../../../config';
import type { GraphLink } from '../../../types';
import { hexToRgb } from '../../../utils/colors';

const FALLBACK_COLOR = '#888888';
const HIGHLIGHT_FALLBACK = '#ffffff';

export type EdgeGeometryGroup = {
  confidentGeometry: BufferGeometry | null;
  inferredGeometry: BufferGeometry | null;
};

export type HighlightedEdgeGeometry = {
  geometry: BufferGeometry | null;
  count: number;
};

/**
 * Build BufferGeometry + vertex colors for a set of edges,
 * split by confidence (confident vs inferred).
 */
export function buildEdgeGroups(
  links: GraphLink[],
  positions: Float32Array,
  nodeIndex: Map<string, number>
): EdgeGeometryGroup {
  const confident: GraphLink[] = [];
  const inferred: GraphLink[] = [];

  for (const link of links) {
    if (link.confidence === 'INFERRED') {
      inferred.push(link);
    } else {
      confident.push(link);
    }
  }

  function buildGeometry(group: GraphLink[]) {
    const verts = new Float32Array(group.length * 6);
    const colors = new Float32Array(group.length * 6);

    for (let i = 0; i < group.length; i++) {
      const si = nodeIndex.get(group[i]!.source)!;
      const ti = nodeIndex.get(group[i]!.target)!;
      verts[i * 6] = positions[si * 3]!;
      verts[i * 6 + 1] = positions[si * 3 + 1]!;
      verts[i * 6 + 2] = positions[si * 3 + 2]!;
      verts[i * 6 + 3] = positions[ti * 3]!;
      verts[i * 6 + 4] = positions[ti * 3 + 1]!;
      verts[i * 6 + 5] = positions[ti * 3 + 2]!;

      const relation = group[i]!.relation;
      const hex = relationPalette[relation] ?? FALLBACK_COLOR;
      const rgb = hexToRgb(hex);
      colors[i * 6] = rgb[0];
      colors[i * 6 + 1] = rgb[1];
      colors[i * 6 + 2] = rgb[2];
      colors[i * 6 + 3] = rgb[0];
      colors[i * 6 + 4] = rgb[1];
      colors[i * 6 + 5] = rgb[2];
    }

    const geometry = new BufferGeometry();
    geometry.setAttribute('position', new Float32BufferAttribute(verts, 3));
    geometry.setAttribute('color', new Float32BufferAttribute(colors, 3));
    return geometry;
  }

  return {
    confidentGeometry: confident.length > 0 ? buildGeometry(confident) : null,
    inferredGeometry: inferred.length > 0 ? buildGeometry(inferred) : null
  };
}

/**
 * Build a single BufferGeometry for edges connected to the selected node,
 * colored by relation type for highlighting.
 */
export function buildHighlightedEdgeGeometry(
  links: GraphLink[],
  positions: Float32Array,
  nodeIndex: Map<string, number>,
  selectedNodeId: string
): HighlightedEdgeGeometry {
  const connectedEdges = links.filter(
    (l) => l.source === selectedNodeId || l.target === selectedNodeId
  );

  if (connectedEdges.length === 0) return { geometry: null, count: 0 };

  const verts = new Float32Array(connectedEdges.length * 6);
  const colors = new Float32Array(connectedEdges.length * 6);

  for (let i = 0; i < connectedEdges.length; i++) {
    const si = nodeIndex.get(connectedEdges[i]!.source)!;
    const ti = nodeIndex.get(connectedEdges[i]!.target)!;
    verts[i * 6] = positions[si * 3]!;
    verts[i * 6 + 1] = positions[si * 3 + 1]!;
    verts[i * 6 + 2] = positions[si * 3 + 2]!;
    verts[i * 6 + 3] = positions[ti * 3]!;
    verts[i * 6 + 4] = positions[ti * 3 + 1]!;
    verts[i * 6 + 5] = positions[ti * 3 + 2]!;

    const relation = connectedEdges[i]!.relation;
    const hex = highlightedEdge.useRelationColor
      ? (relationPalette[relation] ?? HIGHLIGHT_FALLBACK)
      : HIGHLIGHT_FALLBACK;
    const rgb = hexToRgb(hex);
    colors[i * 6] = rgb[0];
    colors[i * 6 + 1] = rgb[1];
    colors[i * 6 + 2] = rgb[2];
    colors[i * 6 + 3] = rgb[0];
    colors[i * 6 + 4] = rgb[1];
    colors[i * 6 + 5] = rgb[2];
  }

  const geometry = new BufferGeometry();
  geometry.setAttribute('position', new Float32BufferAttribute(verts, 3));
  geometry.setAttribute('color', new Float32BufferAttribute(colors, 3));

  return { geometry, count: connectedEdges.length };
}
