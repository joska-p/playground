      ...
      for (let x = 0; x < width; x++) {
        ...
        for (let ky = 0; ky < 3; ky++) {
          for (let kx = 0; kx < 3; kx++) {
            ...
            for (let c = 0; c < 3; c++) {
              gradientX[c] = (gradientX[c] ?? 0) + getPixel(source, x + kx - half, y + ky - half, width, height, c) * wx;
              gradientY[c] = (gradientY[c] ?? 0) + getPixel(source, x + kx - half, y + ky - half, width, height, c) * wy;
            }
          }
        }
        const i = (y * width + x) * 4;
