...
const primitives: FrameCallback = (driver, ctx) => {
        ...
        for (let i = 0; i < cols; i++) {
                for (let j = 0; j < rows; j++) {
                        ...
                        if ((i + j) % 2 === 0) {
                                drawCircle(gridColor((i / cols) * 360 + time * 40))(size)(
                                        vec(cx, cy)
                                )(driver);
                        } else {
                                drawRect(gridColor((i / cols) * 360 + time * 40))({
                                        w: size * 1.7,
                                        h: size * 1.7
                                })(vec(cx, cy))(driver);
                        }
...
const paths: FrameCallback = (driver, ctx) => {
        ...
        const cx = width / 2;
        const cy = height / 2;
        const spin = (point: Vec2): Vec2 =>
                apply2d(compose2d(translation2d(cx, cy), rotation2d(time * 0.5)))(point);
...
const text: FrameCallback = (driver, ctx) => {
        ...
        driver.drawText(
                { fill: '#e2e8f0', fontSize: 56, align: 'center', baseline: 'middle' },
                'Pixelate2D',
                vec(width / 2, height / 2 - 48 + bob)
        );
        drawText('drawText(text)(color)(size)(position)(driver)')('#94a3b8')(15)(
                vec(width / 2, height / 2 + 2)
        )(driver);
        drawText(`fps ${String(Math.round(fps))} · frame ${String(frameCount)}`)('#fbbf24')(14)(
                vec(width / 2, height / 2 + 30)
        )(driver);
        withCamera({ x: 0, y: 0, zoom: 1 })((d) => {
                drawText('screen-space HUD via withCamera — ignores pan/zoom')('#4ade80')(13)(
                        vec(12, height - 16)
                )(d);
        })(driver, ctx);
};
...
const interactive: FrameCallback = (driver, ctx) => {
        ...
        drawText(
                'WASD moves the dot · space pulses · the pink dot tracks the pointer in world space'
        )('#cbd5e1')(13)(vec(12, height - 16))(driver);
        drawText(`world ${world.x.toFixed(0)}, ${world.y.toFixed(0)}`)('#cbd5e1')(13)(
                vec(12, height - 36)
        )(driver);
};
