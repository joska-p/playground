...
function printSummary(): void {
  ...
  const totalTokens = records.reduce((sum: number, r: UsageRecord) => sum + r.inputTokens + r.outputTokens, 0)
  ...
  for (const [provider, { cost, tokens }] of Object.entries(providerTotals)) {
    console.log(`${chalk.bold(provider.padEnd(15))} $${cost.toFixed(4).padEnd(10)} ${tokens.toLocaleString()} tokens`)
  }
  console.log()
}
...
function printList(): void {
  ...
  console.log(chalk.bold.blue('\n=== Usage Records ===\n'))
  console.log(chalk.gray('ID'.padEnd(10)), chalk.gray('Provider'.padEnd(12)), chalk.gray('Model'.padEnd(20)), chalk.gray('Cost'.padEnd(10)), chalk.gray('Tokens'))
...
function printBudgets(): void {
  ...
  if (Object.keys(budgets).length === 0) {
    console.log(chalk.yellow('No budgets configured. Run: ai-usage-tracker set-budget <provider> <amount>'))
    return
  }
  ...
  console.log(chalk.bold.blue('\n=== Budget Status ===\n'))
  console.log(chalk.gray('Provider'.padEnd(15)), chalk.gray('Budget'.padEnd(12)), chalk.gray('Spent'.padEnd(12)), chalk.gray('Remaining'))
  ...
  for (const [provider, budget] of Object.entries(budgets)) {
    ...
    const status = remaining < 0 ? chalk.red(remaining.toFixed(4)) : chalk.green(remaining.toFixed(4))
