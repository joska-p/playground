...
export function CostByProvider() {
  ...
  const getProviderTotals = usageStore(
    (s: { getProviderTotals: () => Record<string, { cost: number; tokens: number }> }) =>
      s.getProviderTotals
  );
  ...
  if (records.length === 0) {
    return (
      <div className="p-4 text-gray-500">
        No usage data yet. Call trackUsage() to start tracking.
      </div>
    );
  }
...
export function SessionSummary() {
  const records = usageStore((s: { records: UsageRecord[] }) => s.records);
  const loadData = usageStore((s: { loadData: () => void }) => s.loadData);
  const getSessions = usageStore(
    (s: { getSessions: (hours: number) => SessionData[] }) => s.getSessions
  );
  ...
  return (
    <div className="p-4">
      <h3 className="mb-2 text-lg font-bold">Session Summary (2-hour windows)</h3>
...
export function RequestLogTable() {
  ...
  const filtered = sorted.filter((r: UsageRecord) => {
    if (filterProvider && !r.provider.toLowerCase().includes(filterProvider.toLowerCase()))
      return false;
  ...
  return (
    <div className="p-4">
      <h3 className="mb-2 text-lg font-bold">Request Log</h3>
...
export function BudgetTracker() {
  ...
  return (
    <div className="p-4">
      <h3 className="mb-2 text-lg font-bold">Budget Tracking</h3>
      <div className="space-y-2">
        {Object.entries(totals).map(([provider, { cost, tokens }]) => (
          <div key={provider} className="rounded border p-3">
