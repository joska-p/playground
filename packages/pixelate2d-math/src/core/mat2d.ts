...
export const mat2d = (a: number, b: number, c: number, d: number, tx: number, ty: number): Mat2D => ({
  a,
  b,
  c,
  d,
 ...
 */
export const multiply2d = (m1: Mat2D) => (m2: Mat2D): Mat2D => ({
...
export const apply2d = (m: Mat2D) => (p: Vec2): Vec2 => ({
...
export const invert2d = (m: Mat2D): Mat2D => {
  ...
  const inv = 1 / det;
  return mat2d(m.d * inv, -m.b * inv, -m.c * inv, m.a * inv, (m.c * m.ty - m.d * m.tx) * inv, (m.b * m.tx - m.a * m.ty) * inv);
};
 ...
 */
export const toMat3 = (m: Mat2D): readonly [number, number, number, number, number, number, number, number, number] => [
  m.a,
  m.b,
  0,
  m.c,
