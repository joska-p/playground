  ...
  └── return { source, final, snapshots }`}
                                ...
                                <div className="border-border bg-muted/30 border-l-utility-5 rounded-lg border border-l-2 p-4">
                                        ...
                                        <p className="text-muted-foreground text-sm">
                                                Double-buffering avoids allocating a new array for
                                                every pixel operation. Two
                                                <code className="mx-1">Uint8ClampedArray</code>{' '}
                                                buffers swap on each pass via O(1) pointer flip.{' '}
                                                <code className="mx-1">snapshot()</code> clones the
                                                current buffer into a new ImageData;{' '}
                                                <code className="mx-1">replaceWith()</code> resets
                                                both buffers when resize or global ops change
                                                geometry.
                                        </p>
                                </div>
                        </div>
        ...
        if (id === 'fusion-scheduler') {
                return (
                        <div className="space-y-6">
                                ...
                                <p className="text-muted-foreground text-sm">
                                        Consecutive pixel-type steps are fused into a single pixel
                                        loop to avoid N full-image passes. The scheduler queues
                                        pixel ops and runs them in one pass when flushed.
                                </p>
...
swap()`}
                                ...
                                <p className="text-muted-foreground text-sm">
                                        Non-pixel operations (neighborhood, global, resize,
                                        snapshot) force a flush before executing, ensuring pending
                                        pixel ops are applied first.
                                </p>
                        </div>
                );
        }
        ...
        if (id === 'tiling') {
                return (
                        <div className="space-y-6">
                                ...
                                <p className="text-muted-foreground text-sm">
                                        Large neighborhood operations are tiled to avoid allocating
                                        full-size temporary buffers. Each tile includes a halo
                                        border of <code className="mx-1">radius</code> pixels so
...
For each tile:
  ...
  3. blitTile({ destination, tile, tileX, tileY, tileWidth, tileHeight, halo })
     → writes only the non-halo center region back`}
                                ...
                                <p className="text-muted-foreground text-sm">
                                        Peak memory drops from{' '}
                                        <code className="mx-1">(width × height)</code> to
                                        <code className="mx-1">(TILE_SIZE + 2×halo)²</code>. The
                                        result is identical to running convolution on the full
                                        image.
                                </p>
                        </div>
                );
        ...
        if (id === 'resize-algorithm') {
                return (
                        <div className="space-y-6">
                                ...
                                <p className="text-muted-foreground text-sm">
                                        Resize uses bilinear interpolation. For each output pixel,
                                        it samples the 4 nearest source pixels and interpolates.
                                </p>
                ...
                return (
                        <div className="space-y-6">
                                ...
                                <div className="border-border bg-muted/30 border-l-utility-1 rounded-lg border border-l-2 p-4">
                                        ...
                                        <p className="text-muted-foreground text-sm">
                                                Stateless worker. Each message rebuilds the registry
                                                from
                                                <code className="mx-1">ALL_MANIPULATIONS</code> and
                                                calls
                                                <code className="mx-1">runPipeline()</code>. Results
                                                are transferred back (zero-copy) via{' '}
                                ...
                                <div className="border-border bg-muted/30 border-l-utility-6 rounded-lg border border-l-2 p-4">
                                        ...
                                        <p className="text-muted-foreground text-sm">
                                                Main-thread pool manager. Workers are lazily created
                                                on first call. If all workers are busy, jobs queue
                                                in FIFO order and dispatch as workers free up. Call
                                                <code className="mx-1">
                                                        pipelineGateway.teardown()
                                                </code>{' '}
                                                on app unmount to terminate workers.
                                        </p>
                                        ...
                                        <table className="w-full text-left text-sm">
                                                ...
                                                <tbody>
                                                        <tr className="border-border">
                                                                ...
                                                                <td className="text-muted-foreground px-4 py-2.5 text-xs">
                                                                        ~16 megapixel default cap.
                                                                        Auto-downscale if source
                                                                        exceeds this value. Also
                                                                        triggers tiled path for
                                                                        neighborhood ops.
                                                                </td>
                                                        </tr>
                                                </tbody>
