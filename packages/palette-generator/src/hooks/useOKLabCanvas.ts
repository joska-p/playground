...
import { oklabTo8bit } from "../utils/oklab";
...
function useOKLabCanvas({ L, displaySize, aRange, bRange }: Params) {
  ...
  const renderOKLabSlice = useCallback((): void => {
    ...
    fillOKLabSliceImageData(img, { L, aRange, bRange }, oklabTo8bit);
    ctx.putImageData(img, 0, 0);
  }, [L, displaySize, aRange, bRange]);
  ...
  const getColorAtClientPosition = useCallback(
    (ev: MouseEvent | { clientX: number; clientY: number }): CssResult => {
      ...
      const [r, g, bl] = oklabTo8bit(L, a, b);
      const hex = "#" + [r, g, bl].map((v) => v.toString(16).padStart(2, "0")).join("");
      const cssOklab = `oklab(${L.toFixed(3)} ${a.toFixed(3)} ${b.toFixed(3)})`;
