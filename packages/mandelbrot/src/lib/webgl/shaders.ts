...
vec2 twoProduct(float a, float b) {
  ...
  float ca = split * a; float ah = ca - (ca - a); float al = a - ah;
  float cb = split * b; float bh = cb - (cb - b); float bl = b - bh;
...
/* ---------- complex df ---------- */
struct cdf { vec2 re; vec2 im; };
cdf cAdd(cdf a, cdf b) { return cdf(dsAdd(a.re, b.re), dsAdd(a.im, b.im)); }
...
void main() {
  ...
  for (int iter = 0; iter < 100000; iter++) {
    ...
    zy = Zr.y + dz.im.x;
    mag2 = zx * zx + zy * zy;
    if (mag2 > BAILOUT) { escaped = true; break; }
