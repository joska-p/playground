...
export function MandelbrotViewer() {
        ...
        useEffect(() => {
                ...
                const maybeRecompute = () => {
                        ...
                        const prec = precisionForZoom(view.zoom);
                        const dxPx =
                                (toNum(reprecision(view).cx, prec) - toNum(ref.cx, prec)) / spacing;
                        const dyPx =
                                (toNum(reprecision(view).cy, prec) - toNum(ref.cy, prec)) / spacing;
                        ...
                        <div className="flex h-dvh w-full items-center justify-center bg-background p-8">
                                <div className="max-w-md rounded-lg border border-border bg-card p-6 text-card-foreground">
                                        ...
                                        <p className="text-sm leading-relaxed text-muted-foreground">
                                                {error} This visualizer needs WebGL2 with
                                                float-texture support. Try a recent version of
                                                Chrome, Firefox, or Safari on a device with GPU
