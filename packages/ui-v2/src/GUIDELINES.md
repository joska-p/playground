...
1. **Mobile-first.** Base classes (no breakpoint prefix) are the complete,
   working experience. `landscape:` / `sm:` prefixes only ever *add*
...
*and* text color *and* focus ring). `variant` is a `cva` variant key,
...
Used when only *one* value needs to change — a focus ring, an accent
...
```css
/* globals.css */
.badge-soft { background: color-mix(in srgb, var(--_color) 15%, transparent); color: var(--_color); }
.toggle:checked { background: var(--_color, var(--primary)); }
.card-interactive:has(.card-actions:hover) { box-shadow: ..., 0 0 0 2px color-mix(in srgb, var(--_color, var(--primary)) 12%, transparent) !important; }
```
...
```tsx
<Badge variant="secondary" appearance="soft" dot>active</Badge>
```
...
```tsx
<Input variant="primary" leadingIcon={<Search />} placeholder="search..." />
```
...
```tsx
<Accordion>
  <AccordionItem title="what makes this different?" open>...</AccordionItem>
...
<Tabs value={tabs.value} onValueChange={tabs.setValue} variant="primary">
  <TabsList>
...
```tsx
<Popover trigger={<Button variant="primary">profile</Button>} widthClassName="w-60" align="center">
  <p>...</p>
</Popover>
```
...
```tsx
<Alert variant="warning" title="unsaved changes" description="may be lost." />
```
...
- [ ] `landscape:`/`sm:` classes only *add* — removing them should
