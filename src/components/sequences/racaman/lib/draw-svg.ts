...
const calculateViewBox = (sequence: number[], containerSize: { width: number; height: number }) => {
...
const generateArcPath = (radius: number, value: number, height: number, clockwise: boolean): string => {
  const sweepFlag = clockwise ? 1 : 0;
  return ` A ${radius} ${radius} 0 0 ${sweepFlag} ${value} ${height / 2}`;
};

const generatePath = (sequence: number[], height: number): string => {
  return sequence.reduce(
    (acc, value, index) => {
      ...
      const clockwise = (index % 2 === 0 && previousValue > value) || (index % 2 !== 0 && previousValue < value);
...
const draw = (svg: SVGSVGElement, sequence: number[], containerSize: { width: number; height: number }) => {
  resetSvg(svg);
