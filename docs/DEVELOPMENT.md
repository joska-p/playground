...
We use a dual-hosting strategy to separate experimentation from production:

- **Development (The Playground)**: Hosted on **GitHub Pages**. 
  - Triggered by pushes to the `develop` branch.
  - URL: `https://joska-p.github.io/playground`
- **Production (The Lab)**: Hosted on **Vercel**.
  - Triggered by merges into the `main` branch.
  - URL: `https://playground-ten-sand.vercel.app`

### Environment-Aware Config
The `astro.config.mjs` dynamically detects the environment using `process.env.VERCEL` and `process.env.GITHUB_ACTIONS` to set the correct `base` path and `site` URL.
