...
export default function (pi: ExtensionAPI) {
  ...
  function tryCompact(
    ...
    if (isCompacting) {
      if (ctx.hasUI)
        ctx.ui.notify(
          'compact-smart: a compaction is already in progress',
          'warning'
        );
      return false;
    }
      ...
      if (turnAge < COOLDOWN_TURNS || timeAge < COOLDOWN_MS) {
        if (ctx.hasUI) {
          ...
          const remainingSec = Math.ceil(
            Math.max(0, COOLDOWN_MS - timeAge) / 1000
          );
          ctx.ui.notify(
    ...
    ctx.compact({
      ...
      onError: (err: Error) => {
        ...
        if (ctx.hasUI)
          ctx.ui.notify(
            `compact-smart: compaction failed — ${err.message}`,
            'error'
          );
      }
    });
  ...
  pi.registerCommand('compact-now', {
    description:
      'Trigger compaction immediately. Append custom instructions, or use ? to query stats.',
      ...
      if (trimmed === '?') {
        ...
        const pct =
          typeof tokens === 'number'
            ? ((tokens / contextWindow) * 100).toFixed(0)
            : '?';
        ctx.ui.notify(
