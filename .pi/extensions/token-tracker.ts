...
function updateFooter(ctx: ExtensionContext) {
  ...
  const costStr =
    sessionUsage.costTotal > 0
      ? ' ' + theme.fg('dim', fmtCost(sessionUsage.costTotal))
      : '';
  ctx.ui.setStatus('token-tracker', text + costStr);
}

function notifyTurnUsage(
  ...
  line.push(
    ...
    theme.fg('text', fmtNumber(sessionUsage.total)),
    theme.fg('dim', ')')
  );
    ...
    handler: async (args, ctx) => {
      ...
      const avgTotal =
        sessionUsage.turns > 0
          ...
          : '—';
      lines.push(
        `  ${theme.fg('dim', 'Turns:')}          ${sessionUsage.turns}`
      );
      lines.push(
        `  ${theme.fg('dim', 'Avg per turn:')}    ${fmtNumber(Number(avgTotal))} tokens`
      );

      // Tokens
      lines.push('');
      lines.push(
        `  ${theme.fg('dim', 'Input tokens:')}     ${fmtNumber(sessionUsage.input)}`
      );
      lines.push(
        `  ${theme.fg('dim', 'Output tokens:')}    ${fmtNumber(sessionUsage.output)}`
      );
      lines.push(
        `  ${theme.fg('dim', 'Total tokens:')}     ${theme.fg('text', fmtNumber(sessionUsage.total))}`
      );
      ...
      if (sessionUsage.costTotal > 0) {
        lines.push('');
        lines.push(
          `  ${theme.fg('dim', 'Estimated cost:')}  ${theme.fg('warning', fmtCost(sessionUsage.costTotal))}`
        );
      }
      ...
      if (ctxUsage?.tokens != null) {
        lines.push('');
        lines.push(
          `  ${theme.fg('dim', 'Current context:')}  ${fmtNumber(ctxUsage.tokens)} tokens`
        );
      }
