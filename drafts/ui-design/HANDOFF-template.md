...
- *Exemple :* Les `slate-*` et `neutral-*` dans `AtlasControls.tsx` ont été remplacés par `surface-raised` et `foreground-dim`.
- *Exemple :* Le `Switch` utilise désormais `text-primary-foreground` pour suivre la palette OKLCH.

## ⚠️ Dette / Sujets en Suspens (Bypass pragmatique)
- *Exemple :* Test de `RandomArt` désactivé temporairement car il s'appuyait sur un ancien composant purgé. À traiter en Phase 3.
