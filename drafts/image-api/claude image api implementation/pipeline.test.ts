...
import { Pipeline, registerManipulation, setConfig, registry } from '../index.js'
import type { PixelFn, NeighborhoodFn, WholeImageFn } from '../types.js'
...
function makeImageData(width: number, height: number, fill: [number, number, number, number] = [100, 150, 200, 255]): ImageData {
    ...
    assert.throws(
      () =>
        registerManipulation({
          id: 'bad-neighborhood',
          type: 'neighborhood',
          fn: neighborFn((src, dest) => { dest.set(src) }),
        }),
      /radius/
    )
  })
