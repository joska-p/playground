...
2. **Marques de repère sémantique** (`ScreenPoint` vs `WorldPoint`) : toute paire `{x, y}` finie est un point écran ET un point monde ; la marque encode *le rôle*, pas une propriété. Deux conséquences :
