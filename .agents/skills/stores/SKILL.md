...
- **Async orchestration** lives in `actions.ts` as a plain async function — read `getState()` _before_ `await`.
...
- When it does get hard to scan, split progressively, only as far as needed:
    ...
    2. Only if `actions.ts`/`selectors.ts` itself gets complicated: split _that_ file into its own subdirectory — one file per exported function (e.g. `useRows.ts` → `useRows()`), or grouped by concern.
