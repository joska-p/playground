 ...
 *
 * Pure-ish helpers split out from `hook.mjs` so unit tests can exercise config parsing, finding
 * filtering, dedup, render, and cache logic without spawning a subprocess. `hook.mjs` itself is the
 * thin stdin/stdout shim.
 *
 * Public surface (everything exported is part of the contract): ENVELOPE_PREFIX, ALLOWED_EXTS,
 * ACK_EXTS, SENSITIVE_PATH, GENERATED_PATH, TRUTHY truthy(value) readConfig(cwd) / DEFAULT_CONFIG /
 * getConfigPath(cwd) / getLocalConfigPath(cwd) resolveProjectPlatform(cwd) /
 * isNativePlatform(platform) normalizeIgnoreValue(value) readCache(cwd) / persistCache(cwd, cache)
 * / resolveCacheCwd(primaryFile, sessionCwd) bumpEditCount(cache, sessionId, filePath) -> number
 * touchFile(cache, sessionId, filePath) suppressionNotice(filePath) filterFindings(findings,
 * content, ext, config) ADVISORY_RULES / isAdvisoryFinding(finding) IMMEDIATE_TIER_RULES /
 * splitFindingsByTier(findings) / perEditTieringActive(config, harness)
 * matchConfiguredExtension(filePath, extensions) dedupeAgainstCache(findings, cache, sessionId,
 * filePath) renderTemplate(findings, filePath, config, opts) renderCleanAck(filePath, opts) /
 * renderPendingAck(filePath, known, opts) appendDesignSystemNote(text, scanOptions) /
 * appendDesignSystemNoteOnce(text, scanOptions, cache, sessionId, config)
 * designNoteReserve(scanOptions, cache, sessionId) footerModeForSession(cache, sessionId) /
 * commitFooterShown(cache, sessionId, text) shouldEmitAckForFile(filePath, config?)
 * writeAuditLog(env, entry) loadDetector() -> Promise<{ detectText, detectHtml }>
 * matchesAnyGlob(filePath, globs) normalizeScanTargets(primaryTargets, projectCwd) runHook(deps) ->
 * { exitCode, stdout, audit, reason? } runStopHook(deps) -> { exitCode, stdout, audit, emission? }
 *
 * Design notes:
 *
 * - All errors are swallowed at the runHook seam. The detector throwing must never break a turn. See
 *   PRD §5 "Failure modes".
 * - Cache shape is JSON-friendly; we gc the oldest sessions when there are more than 8 to keep file
 *   size predictable across long-lived projects.
 * - The detector loader looks for `detector/detect-antipatterns.mjs` next to this file first (built
 *   skill layout) and falls back to the repo root's `cli/engine/detect-antipatterns.mjs` (running
 *   from source).
 */
...
/**
 * Run the hook with explicit dependencies. Returns a result object: { exitCode, stdout, audit,
 * reason? }
 *
...
/**
 * Run the Stop-event deep pass: the FULL detector rule set over every UI file touched this session,
 * surfaced once, deduped against everything the per-edit hook already reported. Same result
 * contract as runHook(): { exitCode, stdout, audit, emission? }
 *
 * Never throws; exits silent (and fast) when the session touched no UI files. Output goes out on
 * the harness's Stop continuation channel: Claude Code and Grok Build read
 * hookSpecificOutput.additionalContext, Codex takes a decision: "block" whose reason becomes the
 * continuation prompt. Either way the findings reach the model and the conversation continues so it
 * can act.
 */
