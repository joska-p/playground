#!/usr/bin/env node
/**
 * API image generation fallback: renders a mock or world board with the user's own OpenAI key when
 * the harness has no native image generation.
 *
 * Context.mjs reports availability (it checks OPENAI_API_KEY); harness-native generation always
 * wins when present. This uses gpt-image-2 and spends the user's API credit (roughly $0.05-0.25 per
 * image at default quality), so the skill states that before the first call in a session.
 *
 * Node generate-image.mjs --prompt "..." --out mock.png [--size 1536x1024] [--quality medium] node
 * generate-image.mjs --prompt-file prompt.txt --out mock.png node generate-image.mjs --prompt "..."
 * --out mock.png --ref screenshot.png [--ref more.png]
 *
 * --ref anchors generation on input image(s) via the edits endpoint: pass a captured screenshot of
 * a representative existing page when comping a new surface for an established world, so the
 * identity comes from the real UI.
 */
