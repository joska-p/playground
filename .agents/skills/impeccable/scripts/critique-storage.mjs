 ...
 *
 * Each critique run writes a per-target snapshot to .impeccable/critique/<timestamp>__<slug>.md
 * with a small YAML frontmatter carrying the score + P0/P1 counts.
 *
 * The polish workflow reads the latest matching snapshot at start as its fix backlog. No other
 * skill auto-reads critique output.
 *
 * The slug is derived mechanically from the _resolved_ primary artifact (file path or URL), never
 * from the user's natural-language phrasing. Slug stability across runs is what lets the trend
 * display work.
 *
 * CLI entry points (called from skill instructions): node critique-storage.mjs slug
 * <resolved-target> node critique-storage.mjs write <slug> <snapshot-body-file> node
 * critique-storage.mjs latest <slug> node critique-storage.mjs trend <slug> [limit]
 *
 * Note: there is intentionally no `ignore` subcommand. ignore.md is a plain markdown file; the
 * model reads it directly with its file-read tool. This helper only exists for operations the model
 * can't trivially do inline (normalizing paths, generating filenames, globbing + parsing
 * frontmatter).
 */
...
/**
 * Mechanically derive a slug from a resolved target. Returns null if the input doesn't look like a
 * stable identifier (empty, project root, etc).
 *
 * Accepts file paths and URLs. The model resolves "the homepage" to a concrete artifact before
 * calling this — we never slug a natural-language phrase.
 */
/**
 * Filename-safe UTC ISO timestamp: hyphens for separators, trailing Z. Plain colons aren't allowed
...
/**
 * Write a snapshot for `slug`. `meta` carries the small structured frontmatter keys read back by
 * readTrend(). `body` is the human-readable critique report (everything below the frontmatter).
 *
...
/**
 * Return the most recent snapshot for `slug`, or null. Polish reads this to find its fix backlog
...
/**
 * Return the last `limit` snapshots' frontmatter, oldest → newest. Critique appends a one-line
 * trend to its output using this.
 */
