 ...
 *
 * Injected into the user's page via <script src="http://localhost:PORT/live.js">. The server
 * prepends window.**IMPECCABLE_TOKEN** and window.**IMPECCABLE_PORT** before this code.
 *
 * UI: a single floating bar that morphs between three states - configure (pick action + go),
 * generating (progressive dots), and cycling (prev/next + accept/discard). Feels like Spotlight,
 * not a modal.
 */
(function () {
    'use strict';
    ...
    function showManualApplyDecision(msg) {
        ...
        storeManualApplyState(count, {
            ...
            repairAttempt:
                numberOrNull(msg?.repair?.attempts) || numberOrNull(msg?.repair?.attempt) || 3,
    ...
    function canRestoreManualEditElement(el, op) {
        ...
        return (
            normalizeManualContextText(el.textContent) === normalizeManualContextText(op.newText)
        );
    }
    ...
    function queryManualEditRef(ref) {
        ...
        for (let index = 0; index < parts.length; index += 1) {
            ...
            current =
                children.find((child) => elementMatchesManualRefSegment(child, segment)) || null;
    ...
    async function mountSvelteComponentVariant(variantNum) {
        ...
        const modulePath =
            String(moduleBase).replace(/\/+$/, '') + '/v' + variantNum + '.' + extension;
    ...
    async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
        ...
        try {
            ...
            abortSvelteComponentInjection(sessionId, {
                ...
                url: manifestPath,
                message:
                    'Could not read the variant manifest. ' + (err?.message || 'Unknown error'),
                previewFile: manifestPath
            });
        }
    }
    ...
    /**
     * Persistent failure surface. Replaces the old 5s toast: a toast that disappears while the
     * session is unusable is indistinguishable from no feedback at all, and the wipe that came with
     * it deleted the only handle on a session the server still considered live.
     */
     ...
     * further event is coming, so the read retries a few times and then surfaces recovery. Callers
     * without the flag may be mid-generation and wait indefinitely for the real completion.
     */
    ...
    function startScrollLock(sessionId, initialTargetY, initialAnchorTop) {
        ...
        window.addEventListener(
            ...
            (e) => {
                if (
                    ['PageDown', 'PageUp', ' ', 'End', 'Home', 'ArrowDown', 'ArrowUp'].includes(
                        e.key
                    )
                )
    ...
    function shouldUseAncestorCropShaderProxy(el) {
        ...
        if (isFrameworkComponentPreviewMode(currentPreviewMode) || svelteComponentSession)
            return true;
...
void main() {
  ...
  gl_FragColor = vec4(mix(ground, u_accent, dotAmt), tex.a);
    ...
    /**
     * A durable server session this page can adopt when the browser has no local record of it.
     * Requires an explicit pageUrl match: a summary with no page is not evidence that it belongs to
     * THIS page, and adopting it would hijack an unrelated route.
     */
    ...
    /**
     * Send is visible once the pill is open for typing and enabled once there is something to send.
     * It disappears entirely while a steer is in flight, the same way the mic does: a second submit
     * during the lock has nowhere to go.
     */
    ...
    /**
     * A submitted steer is queued, not ignored, whenever no poll is parked and the browser knows a
     * generation is in flight: the agent holds the lease and will not see the steer until it
     * finishes. Saying so beats three dots that look identical to a lost request.
     */
    ...
    /**
     * Two minutes of silence has three different causes and only one of them is "live-poll is not
     * running". Naming the wrong one sends the user to restart a poll loop that was never the
     * problem.
     */
    ...
    /**
     * True while the browser is waiting on work it already handed to the agent. In these states a
     * quiet poll indicator means "busy", not "absent".
     */
    ...
    /**
     * Derived at read time, not cached: which of the two reasons applies depends on the live state,
     * which moves between the 5s status polls. The truthiness is the same either way, so the
     * indicator's visuals can stay driven by the cached value while the wording stays current.
     */
    ...
    /**
     * The empty state has to say which emptiness it is. `present:false` (no DESIGN.md at all) is
     * handled upstream in renderDesignBody; everything here means the helper found a design system
     * and this panel found nothing in it worth drawing. Telling that user "no design system data"
     * reads as "your DESIGN.md is missing" and sends them to write a file they already have.
     */
