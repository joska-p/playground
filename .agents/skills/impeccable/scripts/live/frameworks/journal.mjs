 ...
 *
 * Injection writes into the user's source tree: generated components, a Nuxt client plugin, marker
 * blocks inside a layout, a patched CSP meta tag. The clean path removes all of it on stop. The
 * unclean paths do not:
 *
 * - The dev server is SIGKILLed, so `--remove` never runs;
 * - The project changes shape between start and stop (a nuxt.config appears, a package.json is
 *   edited), so detection resolves a different framework and the old framework's artifacts are
 *   nobody's business;
 * - Stop runs from a different directory than start did.
 *
 * So every inject records what it wrote to `.impeccable/live/inject-journal.json` before the next
 * one runs, and both inject and `--remove` reconcile that record against the tree.
 *
 * **The journal is a claim of ownership, not a to-do list.** Healing an artifact only ever removes
 * what still carries our marker; a generated file the user has since replaced, or a layout they
 * have since un-patched by hand, is dropped from the journal untouched.
 *
 * **Path resolution is appRoot-relative.** Live entry scripts chdir onto the roots manifest
 * (`enterLiveRoot`) before doing anything, so a journal written by a session started in the app
 * root is found by a stop issued from any directory inside the repo.
 */
...
/**
 * Record the artifacts an injection just wrote. Replaces any previous record: callers heal first
 * (see healInjectJournal), so nothing survivable is lost.
 */
 ...
 *
 * `keep` is the set of paths the current operation legitimately owns — the artifacts an inject is
 * about to (re)write. Everything else in the journal is an orphan of a session that is gone, and
 * gets healed. This keeps a repeat inject byte-idempotent: the artifacts it is about to rewrite are
 * kept, not torn down and rebuilt.
 *
 * Returns `{ healed, kept }`. `healed` lists only artifacts whose file was actually changed or
 * removed, so callers can stay silent when nothing was orphaned. Idempotent: a second call finds an
 * empty journal.
 */
