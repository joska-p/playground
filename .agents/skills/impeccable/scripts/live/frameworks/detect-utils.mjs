 ...
 *
 * Every helper here is cheap and failure-tolerant: detection runs on every inject, against project
 * trees that may be half-installed, so a missing or malformed file means "not this framework",
 * never a throw.
 */
...
/**
 * Literal (non-glob) entries of `config.files` that exist on disk. Several detectors read the
 * configured injection target as a signal, which is how the bare fixtures — a tree of `.astro`
 * files with no astro.config — still resolve to the framework that authored them.
 */
