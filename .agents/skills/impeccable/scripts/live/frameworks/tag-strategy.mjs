 ...
 *
 * Frameworks without a dedicated adapter get a literal marker-wrapped `<script src>` block written
 * into the entry template named by `.impeccable/live/config.json`. This module owns that block:
 * building it, inserting it at the configured anchor, removing it again, and the
 * Content-Security-Policy meta patch that keeps the cross-origin load allowed.
 *
 * It is deliberately framework-agnostic. Per-framework knowledge (Astro's `is:inline`, for
 * instance) arrives as the `scriptAttrs` argument, resolved from the registry by the caller, so
 * nothing here has to branch on a file extension or a project shape.
 */
...
/**
 * `scriptAttrs` is a pre-rendered attribute string (trailing space included) that the registry
 * supplies for the target file. Astro is the only framework that uses it today: Astro processes
 * `<script>` tags by default and rewrites src to its own bundled URL, so `is:inline ` opts out and
 * the literal external src survives.
 */
...
/**
 * Remove the live script block. Matches either HTML or JSX comment markers regardless of config (so
 * stale tags from a wrong config can still be cleaned).
 *
 * Indent-preserving: captures any whitespace immediately preceding the opener marker and re-emits
 * it in place of the removed block. `insertTag` inserted the block _after_ the original line's
 * indent and _before_ the anchor (e.g. `</body>`), which moved the indent onto the opener line and
 * left the anchor unindented. Replacing the whole block (plus its trailing newline) with just the
 * captured indent hands the indent back to the anchor that follows.
 */
