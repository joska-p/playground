 ...
 *
 * Next takes the generic tag strategy: the App Router's root layout renders `<html>…<body>` in JSX,
 * so the marker-wrapped script block goes in there verbatim. Nothing about injection differs from a
 * plain Vite app, which is why live-inject.mjs never had a Next branch. The entry exists so the
