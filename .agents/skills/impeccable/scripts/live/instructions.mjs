 ...
 *
 * The live scripts, not the reference doc, own situational plumbing: every event printed by
 * live-poll carries an `_instructions` string describing exactly what to do NEXT, with real ids,
 * paths, and line numbers already substituted and only the active path's rules included (a
 * svelte-component session never sees JSX guidance, and vice versa). live.md stays lean: the
 * session contract, harness policy, and design-quality guidance that is not situational (identity
 * lock, variation axes, parameter budgets).
 *
 * Keep these strings imperative, concrete, and short. They are read by an agent mid-session; every
 * sentence must earn its tokens. Instructions are versioned with the scripts, so they cannot drift
 * from behavior the way a hand-maintained doc can.
 */
