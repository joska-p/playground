/**
 * Pure helpers for live-mode insert UI (browser + tests). Kept separate from live-browser.js so
 * insert logic is unit-testable.
 */
...
/**
 * Whether Create is allowed for an insert session. Requires a non-empty prompt OR at least one
 * annotation.
 */
...
/**
 * How the in-flow placeholder should participate in layout. Prefer implicit sizing (flex / %) so
 * row inserts don't inherit the full parent width in px.
 *
 * @returns {{ kind: 'flex'; flex: string; minWidth: number }
 *     | { kind: 'percent' }
 *     | { kind: 'auto' }
 *     | { kind: 'explicit'; width: number }}
 */
