 ...
 *
 * TanStack Start is SSR: there is no static index.html to patch. The document shell is a React
 * component (`shellComponent`/`component`) defined in the root route file, `src/routes/__root.tsx`,
 * which renders `<html>…<body>{children} <Scripts /></body></html>`.
 *
 * A raw `<script src>` placed in that JSX is server-rendered into the streamed HTML, but React's
 * script handling and hydration make it an unreliable place to load a cross-origin dev bundle. So,
 * like the Nuxt and SvelteKit adapters, this keeps the injected code in a dev-only managed
 * component that appends the live script on mount (client-only, after hydration). The adapter
 * mounts that component from the root document and removes it cleanly on stop.
 *
 * The managed component lives OUTSIDE `src/routes/` (in `src/impeccable/`) so the TanStack Router
 * file-based route generator never treats it as a route.
 */
