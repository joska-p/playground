 ...
 *
 * Both scripts need the same thing: find the one project file containing a string (wrap looks for
 * the element's class/id/text, accept looks for the session's `impeccable-variants-start` marker).
 * They had two near-identical copies of the walk, and the copies drifted — same `EXTENSIONS` array
 * declared twice, same `searchDirs` array declared twice, one `realpathSync` guarded by try/catch
 * and the other not. That drift is what #374 had to patch in two places at once.
 *
 * Callers differ only in how they reject a candidate, so that is the one thing this module takes as
 * options (`skipDirs`, `fileFilter`).
 */
 ...
 *
 * `lib` is here for Phoenix, whose templates live in `lib/my_app_web/`. It is an ordering
 * preference rather than a reachability fix: `.` already recurses into `lib`, so the real #374 bug
 * was the extension list, not this array.
 */
 ...
 *
 * `.impeccable` is the critical entry, and it is not cosmetic. Progressive publication stages each
 * revision as `.impeccable/live/artifacts/ <id>-r<n>.<source-ext>`, and those artifacts carry the
 * very marker accept searches for. The walk reaches `.` for any project whose source is not under
 * one of the privileged roots above (this repo's own site lives in `site/pages/`), and
 * dot-directories sort before letters, so the artifact was found _before_ the real file.
 * isGeneratedFile then declined the accept, and the agent fell back to carbonizing several hundred
 * lines of stylesheet by hand.
 */
