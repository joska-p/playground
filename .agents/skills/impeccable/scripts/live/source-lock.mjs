...
/**
 * Remove the lock only if it is still the one this call created. If a sweeper judged our lock stale
 * and another writer replaced it, unlinking here would end _their_ critical section and admit a
 * third writer to the same file.
 */
 ...
 *
 * Age alone cuts both ways: it sweeps a live holder whose critical section outran the timeout (a
 * suspended laptop, a stopped process), letting two writers into the same source file, while still
 * making every accept on a crashed holder's file wait out the full timeout. Asking the OS whether
 * the recorded pid is alive answers both correctly: a dead owner releases at once, and a live owner
 * keeps its lock however long it needs.
 */
