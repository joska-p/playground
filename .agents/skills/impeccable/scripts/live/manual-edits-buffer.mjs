 ...
 *
 * Location: .impeccable/live/pending-manual-edits.json (project-local). Schema: { version: 1,
 * entries: [{ id, pageUrl, element, ops, stagedAt }] }
 *
 * Each entry corresponds to one Save action from the browser. Ops merge by (pageUrl, ref): if the
 * user re-edits the same element before committing, the existing entry's `newText` is replaced and
 * `originalText` is kept (it holds the real source state).
 */
...
/**
 * Merge a new entry into the buffer. For each op in the new entry, if there's already a buffered op
 * for the same (pageUrl, ref), update that op's newText and keep its original originalText (the
 * true source state). Otherwise add the op (creating an entry if needed).
 *
...
/**
 * Remove entries matching a predicate. Returns count of removed _ops_ (not entries) so callers
 * report a unit consistent with truncateBuffer and the pill's per-page op count. Empty entries (no
 * ops left) are also pruned.
 */
...
/** Truncate the buffer to empty (used by discard-all). Returns the count of removed ops. */
