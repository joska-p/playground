/**
 * Live root resolution: the single place that decides which directories a live session operates on.
 * Every live entry script resolves this once at startup (see enterLiveRoot) instead of trusting its
 * ambient cwd, which is how a `cd` used to silently fork the whole system into a second, empty
 * project.
 *
 * Four distinct roots travel together as one manifest:
 *
 * AppRoot what the dev server serves; where live session state, injected adapters, and preview
 * modules live. repoRoot the git boundary (falls back to appRoot outside git). contextRoot the
 * nearest directory from appRoot up to repoRoot carrying PRODUCT.md / DESIGN.md (canonical spot or
 * a fallback dir). sessionRoot <appRoot>/.impeccable/live — durable live state.
 *
 * AppRoot detection keys on dev-server config presence (vite/svelte/next/ astro/nuxt/... config
 * files), not on monorepo brand markers. A nested website/ with vite.config.js wins over a repo
 * root that merely has a package.json. Workspace declarations are one input, not the gatekeeper.
 *
 * The resolved manifest is persisted at <appRoot>/.impeccable/live/roots.json plus a pointer at
 * <repoRoot>/.impeccable/live/app-root.json when the two differ, so a helper invoked from anywhere
 * inside the repo finds the same roots the boot decided on. When several apps in one repo run live,
 * the pointer follows the most recent boot; per-app roots.json files stay put.
 */
...
/**
 * Scan downward (bounded depth) for directories carrying a dev-server config. Used when live boots
 * from a directory that is not itself an app root and no --target narrows the choice: one candidate
 * is auto-picked, several become a selection prompt.
 */
 ...
 *
 * Returns { manifest } on success or { selection } when several candidate apps exist and nothing
 * disambiguates.
 */
...
/**
 * True when the app's live helper server is recorded and its pid is alive. A liveness signal alone
 * misclassifies a REUSED pid (helper died without removing server.json, the OS handed the pid to
 * something else), so the process's command line must also look like a node process; that removes
 * reuse by arbitrary processes. A pid reused by another node process remains a residual false
 * positive, which the multi-app warning and --target escape hatch cover.
 */
...
/**
 * True when the app's durable session store holds a session that is not terminal. With every helper
 * server stopped, this is what distinguishes "the app whose interrupted session the user is trying
 * to recover" from an app that merely booted more recently.
 */
...
/**
 * Resolve the roots for the live session governing `cwd`, preferring a persisted manifest (written
 * by the boot) over fresh detection:
 *
 ...
 *
 * Fresh results are NOT persisted here; only the boot (live.mjs / server startup) writes manifests,
 * so ad-hoc helper invocations cannot mint conflicting truth.
 */
...
/**
 * Consume a `--target <path>` / `--target=<path>` pair from an argv array, returning the value and
 * removing the tokens so downstream flag parsers (which do not know the option) never see them.
 */
...
/**
 * Entry-point guard for live CLI scripts: resolve the governing roots and make appRoot the process
 * cwd so every downstream path derivation agrees with the boot. An explicit `--target <path>` on
 * the helper's command line overrides pointer resolution, which is what disambiguates a repo with
 * several live apps (the multi-app warning names this escape hatch, so it has to actually work on
 * every helper). Returns the manifest. On selection ambiguity it stays in the current directory
 * (the boot flow handles prompting); a malformed --target exits with an error instead of silently
