/**
 * Postcondition scanner for accepted/carbonized source. The carbonize contract used to exist only
 * as prose in reference/live.md; nothing checked that an accept actually left the file clean, so
 * dead param branches, preview attributes, and marker comments accumulated across sessions. This
 * scanner is the mechanical form of that contract. live-complete refuses to mark a carbonize
 * session complete while the file is dirty, and the mechanical Svelte accept runs it on its own
...
/**
 * Scan file text for live-mode leftovers. Returns { clean, findings } where each finding is {
 * marker, line, excerpt, why }.
 */
