/**
 * Canonical inventory of the Live overlay's UI surfaces: one entry per piece of chrome Live mounts
 * on the user's page, with the element ids that make it up.
 *
 * Single source of truth, consumed by:
 *
 * - Skill/scripts/live/browser-script-parts.mjs — serializes this into
 *   window.**IMPECCABLE_LIVE_UI_SURFACES** in the /live.js prelude.
 * - Skill/scripts/live-browser.js — publishes it on window.**IMPECCABLE_LIVE_CHROME_CORE** for
 *   adapters and E2E probes. That file is served raw and injected as a classic <script>, so it
 *   cannot import this module at runtime; it reads the injected global instead, the same path
 *   live/vocabulary.mjs already takes for the command palette.
 * - The private impeccable-site repo — site/components/LiveUiGallery.astro and
 *   tests/live-ui-lab.test.mjs import LIVE_UI_SURFACES at build time and fail the site build when
 *   the Live UI lab has no snapshot for a surface defined here. That guard only guards if it reads
 *   this list rather than a copy the site keeps, so this module must stay importable from Node.
 *   Renaming a key or the module is a breaking change for that build; the list was briefly inlined
 *   into live-browser.js and the site had to parse it back out with a regex.
 *
...
/**
 * The mount contract every Live chrome adapter (DOM, Svelte, ...) satisfies. Published alongside
 * the surfaces on **IMPECCABLE_LIVE_CHROME_CORE**.
 */
