 ...
 *
 * The scaffolder turns the selected block of a route's markup into a detached preview component
 * whose dynamic values arrive as props. The old implementation matched `{...}` with a regex, which
 * flattened control-flow blocks ({#each}, {#if}) into scalar text props and shipped structurally
 * wrong previews. This module uses the app's own svelte compiler (parse with modern: true) and
 * replaces only expressions that are FREE, i.e. reference identifiers not bound by an enclosing
 * template scope:
 *
 ...
 * caller falls back to source-preview mode, which keeps the markup inside the route file where
 * those references still resolve. A wrong preview is worse than a plain one.
 *
 * The compiler is resolved from the APP's node_modules, never bundled: the preview must be parsed
 * by the same svelte version that will compile it.
 */
...
/**
 * Resolve the app's svelte compiler synchronously (svelte 5 ships a CJS compiler build, so
 * createRequire works and the accept/scaffold pipeline stays synchronous). Returns { parse,
 * compile, VERSION } or null.
 */
...
/**
 * Collect the root identifiers an ESTree expression reads. Walks generically; skips non-computed
 * member properties and non-computed/non-shorthand object keys, which are names, not references.
 */
...
/**
 * An expression mixing loop-bound and outer free identifiers (e.g. `{fmt(stage.label)}` where `fmt`
 * lives in the route script) can neither become a prop (the bound part varies per item) nor survive
 * detachment verbatim (the free name is undeclared in the preview and throws at mount, past the
 * compile gate, because globals make it legal to the compiler). Source-preview mode is the only
 * correct home for it.
 */
...
/**
 * Analyze a parsed template fragment. `scopes` is a stack of Sets of bound names; the outermost
 * call passes an empty stack.
 */
        ...
        switch (attr.type) {
            case 'Attribute': {
                ...
                for (const part of parts) {
                    ...
                    if (failOnMixedExpression(part.expression, scopes, analysis, analysis.source))
                        return;
...
/**
 * Describe the repeating item of an each block for browser-side hydration: the item's root element
 * (tag + static classes, used to count live iterations) and the ordered text slots that reference
 * loop bindings.
 */
...
/**
 * Classify a keyed each block's key expression: { keyField } member of the loop item (e.g.
 * `(expense.id)` when the context binds `expense`): browser injects a unique per-index value under
 * that field. {} key is the whole loop item or the index: already distinct per iteration, nothing
 * to inject. { unsupported } free or complex keys: cannot hydrate distinct values.
 */
function classifyEachKey(node) {
    ...
    if (!usesLoopBinding) {
        ...
        return {
            unsupported: 'each key not derived from the loop item requires source-preview mode'
        };
    }
...
/**
 * Describe a fragment's root element for browser presence probing: { tag, classes } of the first
 * RegularElement, or null for text-only fragments (which cannot be probed reliably).
 */
...
/**
 * Analyze a markup block and produce the prop-substituted scaffold markup and the v2 prop contract.
 * Returns { ok: false, reason } when the block needs source-preview mode (parse failure or
...
export function analyzeSvelteMarkup(markup, parse) {
    ...
    for (const entry of analysis.contract) {
        if (entry.kind === 'collection' && entry.item?.nestedUnsupported) {
            return {
                ok: false,
                reason: 'per-item content (nested blocks or expressions) this preview cannot hydrate requires source-preview mode'
            };
        }
    }
...
/**
 * Restore a variant's markup back to route-source form: every free identifier that matches a
 * contract prop is replaced by its original expression. AST-based so `{#each stages as stage}`
 * restores to `{#each data.stages as stage}` even though the prop appears without braces.
 */
...
/**
 * Report [name, start, end] for every free root identifier READ in an expression (skips member
 * properties, object keys, shadowed names).
 */
...
/**
 * Build the preview component's script block from a v2 contract, with defaults that keep an
 * unhydrated mount rendering instead of crashing.
 */
