 ...
 *
 * Async on purpose. This spawns `live-wrap.mjs`, which walks the project's source tree and can take
 * seconds (measured at ~7.6s on a large repo when the element is not found, with a 15s ceiling).
 * The live server is single-threaded and calls this while leasing a poll, so a synchronous spawn
 * froze the whole server for that entire window: Accept and Discard POSTs, SSE progress broadcasts,
 * and every other poll stalled behind it.
 */
