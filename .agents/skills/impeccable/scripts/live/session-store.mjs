...
export function createLiveSessionStore({ cwd = process.cwd(), sessionId } = {}) {
     ...
     * @type {Map<
     *     string,
     *     { snapshot: object; nextSeq: number; journalPath: string; size: number; mtimeMs: number }
     * >}
     */
    ...
    /**
     * The current derived state for a session, from the in-memory cache when the journal has not
     * moved, from the snapshot file when that file is provably current, and from a full replay
     * otherwise.
     */
...
/**
 * Hydrate derived state from a snapshot file, but only when it provably describes the journal as it
 * stands right now. Anything short of an exact byte match on an append-only file means events
 * landed after the snapshot was written, and the caller replays instead.
 */
...
/**
 * `pending` = the agent published and nothing has acked yet, `mounted` = at least one variant
 * reached the DOM, `failed` = the browser reported failures and nothing ever mounted. A single
 * success outranks any number of failures: the user is looking at something.
 */
