 ...
 *
 * Variants are real .svelte components under node_modules/.impeccable-live/<session-id>/. The
 * browser mounts them via Svelte 5 mount(); accept inlines the chosen variant back into the route
 * source with props mapped to original bindings.
 */
...
/**
 * Scaffold a component-preview session. The scaffold is AST-based: the app's own svelte compiler
 * parses the selected markup, control-flow blocks are preserved (an each collection crosses the
 * prop contract as ONE structured prop, its loop body verbatim), and constructs a detached preview
 * cannot support return `{ fallback: 'source-preview', reason }` so the caller keeps the markup
 * inside the route file instead of shipping a wrong preview.
 */
...
/**
 * Seed variant stubs with the source component's rules that already style the selected markup, so
 * variants start from the real cascade (a detached preview inherits none of the route's
 * compile-scoped CSS) instead of reimplementing it blind.
 *
 * Returns { css, supersedable }. `css` is every matching rule (class OR tag matched).
 * `supersedable` holds only the CLASS-matched selectors: those are the accept-time removal
 * candidates. Tag selectors (h1, a, p) style shared elements across the whole route, so they seed
 * the preview but are never candidates for removal.
 */
...
/**
 * Remove every rule whose (normalized) selector list is fully contained in `selectors` from the
 * component's style block, at any at-rule nesting depth. Rules that mix doomed and surviving
 * selectors keep the survivors.
 */
...
/**
 * Merge CSS into a svelte component's top-level style block (created when absent), replacing rules
 * whose selectors match and appending the rest.
 */
 ...
 * passed and the browser's import failed with "Unexpected token '('". Two reasons it cannot close
 * the gap, both structural rather than oversights:
 *
 * - `generate: false` produces no JS to inspect, and generating it here would spend a full codegen
 *   per variant on the publish path.
 * - `loadSvelteCompiler` reaches the compiler through createRequire, which Svelte's export map routes
 *   to a prebuilt CJS build. The dev server imports `src/compiler` instead, and only that path uses
 *   the app's installed printer (esrap). The two can disagree, so even a generated check here would
 *   be checking a different compiler than the one whose output the browser runs.
 *
 * Tests/live-svelte-props-script.test.mjs covers the emitted JS, importing the compiler as ESM so
 * it sees what the dev server sees.
 */
...
/**
 * Snapshot the agent-authored variant files into a fresh revision directory and stamp the manifest.
 * Called by the server on every publish (`done` reply) for a component session; the browser imports
 * from the revision dir, so the dev server can never serve a stale compile of a republished file.
 */
...
/**
 * Stop-path sweep. The whole `node_modules/.impeccable-live` tree is impeccable-owned and
 * gitignored, so once no session should survive there is nothing left worth keeping: the
 * per-session dirs, the generated `__runtime.js`, and the parent directory all go. The old
 * per-entry loop skipped `__*` entries and the parent, which left the runtime shim and an empty
 * directory in every project that ever ran live mode once.
 */
...
/**
 * Boot-path sweep. A restart must not delete the tree wholesale: sessions recorded in the session
 * store may still be mid-generation. Remove only the session dirs whose id has no active snapshot,
 * then drop `__runtime.js` and the parent directory when nothing is left to serve.
 *
