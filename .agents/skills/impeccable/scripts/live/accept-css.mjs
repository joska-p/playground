 ...
 *
 * The old accept path appended the chosen variant's whole <style> body in front of the component's
 * existing rules, which preserved every superseded declaration (the "old divider borders survive
 * the accept" bug) and left dead parameter branches in source. This module makes acceptance a
 * merge:
 *
 * ReconcileCss replace rules whose selectors match, append new ones bakeParamValues collapse --p-*
 * vars and [data-p-*] branches to the user's chosen values, driven by the declared param kinds from
 * params.json (not regex sniffing) pruneUnusedSelectors use the framework compiler's own
 * unused-selector warnings to delete rules the accepted markup no longer references
 *
 * The parser is hand-rolled on purpose: skill scripts run standalone inside user projects and
 * cannot rely on this repo's node_modules. It is a small recursive block parser (comment- and
 * string-aware), not a spec-complete CSS parser; everything it emits round-trips byte-for-byte
 * through raw slices except the rules deliberately changed.
 */
...
/**
 * Parse a stylesheet into a flat tree. Node shapes: { type: 'rule', prelude, body, start, end,
 * preludeStart } { type: 'at', name, prelude, children|body, start, end } (children when the block
 * contains rules: media/supports/layer/container/scope) { type: 'comment', text, start, end }
 */
...
/**
 * Scan for the next structural boundary: the `{` opening a block, or the `;` ending a block-less
 * at-statement, whichever comes first (string- and comment-aware). Returns { kind: 'block' |
 * 'statement' | 'none', index }.
 */
...
/**
 * Merge variant CSS into existing CSS. Rules whose (at-context, normalized selector) match an
 * existing rule REPLACE that rule's body in place; new rules append at the end under their
 * at-context. Returns { css, replaced, appended }.
 */
...
/**
 * Replace every `var(--p-<id>, fallback)` / `var(--p-<id>)` occurrence with a literal value.
 * Paren-aware: fallbacks containing calc()/nested vars are handled, unlike the old `[^)]+` regex.
 */
...
/**
 * Strip `[data-p-<id>="value"]` / `[data-p-<id>]` attribute selectors from a selector, deciding
 * survival by the chosen value: returns null when the selector targets a non-chosen branch (drop
 * it), otherwise the selector with the attribute test removed and any emptied :global() wrappers
 * cleaned up.
 */
...
/**
 * Bake chosen parameter values into CSS. `params` is the declared parameter list for the accepted
 * variant (from params.json); `values` maps id -> chosen value (falling back to each param's
 * declared default).
 */
...
/**
 * Remove selectors the framework compiler reports as unused from a full component source.
 * `compileFn` is the app's svelte compile; warnings with code `css_unused_selector` carry character
 * offsets into the source. `skipSelectors` protects selectors that were already unused before the
 * accept: pre-existing dead rules are the user's code, not live-mode debris. Returns { source,
 * removed } where removed lists the pruned selector texts.
 */
    ...
    try {
        ...
        return new Set(
            (warnings || [])
                ...
                .map((w) =>
                    String(componentSource).slice(w.start.character, w.end.character).trim()
                )
        );
    } catch {
        return new Set();
...
/**
 * Remove the selector at [start, end) from its rule. When it is the rule's only selector, remove
 * the whole rule (prelude through closing brace).
 */
...
/**
 * Collect every normalized selector in a CSS text, including inside nested at-blocks. Used by the
 * accept postcondition: a selector present before the accept may only disappear if the compiler
 * reported it unused; anything else means the parser or reconciler damaged the user's file, and the
 * write must be refused rather than silently committed.
 */
