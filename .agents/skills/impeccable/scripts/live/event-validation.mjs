/**
 * Shared event validation for the live helper server. Extracted for unit testing (insert mode
 * rules).
 */
...
/**
 * Mount acknowledgements are the browser's answer to "did the thing you published actually render".
 * They are validated strictly because the render truth in the session snapshot is built from them:
 * a malformed ack that slid through would report a variant as mounted that never was.
 */
