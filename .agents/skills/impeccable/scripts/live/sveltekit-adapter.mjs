 ...
 *
 * SvelteKit must not be patched through src/app.html. That file is a document template, not
 * framework-owned component chrome. The adapter keeps SvelteKit work limited to mounting a dev-only
 * shadow host from +layout.svelte; the actual live UI remains the shared plain-DOM browser chrome.
 */
...
/**
 * The import specifier carries a token-derived revision query. The adapter component embeds the
 * helper token, and Vite (client AND SSR) can keep serving a stale compiled module after the file
 * is rewritten on a helper restart; the browser then requests /live.js with a rotated-out token and
 * gets a 401 with no picker. A changed specifier is a different module id, which no cache
 * survives.
 */
