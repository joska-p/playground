#!/usr/bin/env node
/**
 * Context-signals gatherer for the bare Impeccable invocation (no-argument) path. Collects cheap,
 * deterministic signals about the current project and emits them as JSON.
 *
 * It does NOT score or rank. The agent reasons over the raw signals using its knowledge of the
 * command catalog (see SKILL.md routing rule 1). Deliberately light: no LLM calls, no detector run
 * (`npx impeccable detect` is heavier and opt-in), no file writes. Every probe is best-effort and
 * never throws; the output is always valid JSON.
 *
 * Signals:
 *
...
function gitSignals(cwd) {
    ...
    const fromStatus = run(['-c', 'core.quotepath=false', 'status', '--porcelain'], {
        trim: false
    });
    let changed = [];
    if (fromDiff) {
...
/**
 * Local paths the agent should point the bundled detector at — never a URL. A URL means a costly
 * Puppeteer browser render, and a probed dev-server port may not even belong to this project. An
 * HTML _file_ or a source tree is scanned by the cheap, jsdom-free static engine. This script does
 * NOT run the detector; it just surfaces the target(s) so the agent can run `node
 * <scripts>/detect.mjs --json <targets>` and fold the hits in.
 */
