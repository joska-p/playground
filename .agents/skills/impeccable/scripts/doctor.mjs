 ...
 *
 * Node doctor.mjs # human-readable report node doctor.mjs --json # machine-readable, for the skill
 * command node doctor.mjs --fix # apply the mechanical migrations only node doctor.mjs --target
 * <path> # pick a monorepo workspace
 *
 * The boot check in context.mjs reports what a session can afford to measure. This runs everything:
 * git drift, per-workspace sweep, ignore-list validation against the live rule registry, hook
 * script resolution.
 *
 * `--fix` is deliberately narrow. It performs only the migrations marked severity 'auto', the ones
 * with no judgment in them: stamp the product record, move a sidecar out of a retired location.
 * Anything that needs an answer from the user (a platform value, whether an inherited record still
 * describes an app, whether a document has drifted from the code) is reported and left alone. Exit
 * code is 0 unless the run itself failed; findings are not errors.
 */
...
/**
 * Apply the migrations that carry no decision. Returns what was done and what was deliberately left
 * for the user.
 */
