/**
 * Context loader: prints PRODUCT.md, DESIGN.md when present, the matching persisted surface brief
 * when one can be resolved, and native-platform guidance selected from PRODUCT.md. It prints a
 * `NO_PRODUCT_MD:` message when no PRODUCT.md is found anywhere. The skill keys off that message to
 * branch: from-scratch build requests (plus init / teach / shape) and clear build/shape intent
 * divert into the init flow, while scoped commands proceed using the existing code as context.
 *
 * Path resolution (first match wins):
 *
 * 1. Active project root, if PRODUCT.md or DESIGN.md is there. An explicit --target selects the active
 *    project: the workspace child in a monorepo, or the nearest directory around the target
 *    carrying canonical context files in an ordinary repo (issue #376).
 * 2. Active project .agents/context/ then docs/
 * 3. Repo root context, using the same order, as a per-file fallback whenever the active project is
 *    nested below it (a repo counts as a monorepo when a package manager declares workspaces, or
 *    `.impeccable/config.json` declares `projectRoots`)
 * 4. $IMPECCABLE_CONTEXT_DIR (absolute or cwd-relative) — power-user escape hatch, only consulted when
 *    defaults are empty
 * 5. Active project root as a "nothing found" default
 *
 * `resolveContextDir()` and `loadContext()` are also exported for the server-side scripts
 * (live.mjs, live-server.mjs) that need the structured shape rather than the markdown block.
 */
...
function hasFallbackWorkspaceChildren(dir) {
    for (const name of MONOREPO_FALLBACK_PROJECT_DIRS) {
        ...
        if (
            entries.some(
                (entry) => entry.isDirectory() && !isIgnoredWorkspaceDiscoveryDir(entry.name)
            )
        )
            return true;
    }
...
/**
 * Best-effort evidence that the project already has an incumbent visual implementation. DESIGN.md
 * is documentation, not the only source of design authority: real tokens, chosen type, and a
 * component system in code must not be mistaken for a greenfield identity merely because the
 * document is absent.
 *
 * The scan is deliberately bounded and conservative. A package.json or one empty scaffold component
 * is not enough; a tokenized stylesheet, an authored HTML surface, or several styled UI components
 * is.
 */
...
/**
 * Read the first non-empty line under a bare `## <heading>` section of PRODUCT.md (for example `##
 * Platform`). Returns null when the section is absent. The heading match is exact (`\s*$`) so
 * near-miss near-miss headings don't shadow the real field.
 */
...
/**
 * Pull the platform (`web`, `ios`, `android`, or `adaptive`) out of PRODUCT.md by looking for a `##
 * Platform` section and reading the first non-empty line that follows it. `adaptive` is for
 * cross-platform apps (Flutter, React Native) that ship both iOS and Android from one codebase; a
 * line that names both targets (e.g. `ios, android`) is also read as `adaptive`. Returns null when
 * the file is legacy / platform-less, which the skill treats as `web` (the default the general
...
/**
 * Read the installed skill's own version from the sibling SKILL.md frontmatter (this file lives at
 * `<skill>/scripts/context.mjs`). Returns null when the frontmatter is missing or unreadable.
 */
...
/**
 * Best-effort update directive for the boot output. Returns a string to append or null. Polls the
 * version endpoint at most once per day (cached globally in the user's home dir) and re-surfaces a
 * given version at most once per week so the agent never nags. Opt out entirely with
