#!/usr/bin/env node
/**
 * Visual question server: present a decision to the user as a themed page instead of a plain-text
 * prompt, then block until they answer.
 *
 * The script IS the wait: run it via the shell, it serves the page, prints the URL (and tries to
 * open the default browser), and does not exit until the user chooses. The answer lands on stdout
 * as one line:
 *
 * ANSWER: {"optionId":"...","steer":"..."}
 *
 * Exit codes: 0 answered · 2 timed out, closed without answering, or no browser is available
 * (IMPECCABLE_QUESTION_DISABLED, or a detected CI/headless/remote environment;
 * IMPECCABLE_QUESTION_FORCE=1 overrides detection, --no-open skips it since the caller opens the
 * URL itself).
 *
 * Payload (JSON file via --payload, or stdin): { "title": "Choose the visual world", "question":
 * "The roll assigned Fillmore Handbill. Keep it, take an alternate, or re-roll.", "options": [ {
 * "id": "assigned", // returned verbatim "label": "Fillmore Handbill", "kicker": "THE ROLL", //
 * optional badge; the assigned option leads "lineage": "1966-71 Fillmore ...", // optional
 * "thesis": "one line: the idea this direction owns", // optional "palette": ["#1a2f5e", "oklch(84%
 * .19 80)", ...], // optional, rendered as chips "materials": ["letterpress", "newsprint"], //
 * optional, rendered as tags "viewport": "one line: the first-viewport composition", // optional
 * "case": "one line: the fusion verdict, honest", // optional "verdict": "competitive", // optional
 * routing tier: "wins" | // "competitive" | "declined". Declined cards // render demoted after the
 * full cards: // narrow, quiet, catalog art as a labeled // thumb, "Adopt anyway" instead of "Build
 * // this". Still choosable; never deleted. "kept": "one line: what the direction kept from this
 * declined world", "raised": [ { "from": "challenger-x", "raise": "one line" } ], // assigned card
 * only: donations taken from // declined challengers, rendered as named // raise lines under the
 * identity row "risk": "one line: the honest risk", // optional "body": "fallback prose when the
 * structured fields are absent", "comp": ".impeccable/mocks/decision/assigned.webp", // optional;
 * the card's // full-fidelity direction comp (the legacy // key "sketch" is accepted as an alias).
 * May // not exist yet: the page shimmer-waits and // polls the slot until the file lands, so //
 * serve first and generate after "hero": "https://... or /abs/path.webp", // optional inspiration
 * image; // rides picture-in-picture when a comp exists "board": "https://... or /abs/path.webp" //
 * optional secondary image }, ... ], "reroll": true, // adds a re-roll action (returns
 * {"optionId":"reroll"}) // or { "registers": ["safer", "bolder"] } to add // the register steers
 * beside it: the answer then // carries "register" and the agent re-runs // concept-seed with
 * --register <value> "canon": true, // adds the "Play it straight" standing exit; // direction
 * rounds only (returns {"optionId":"canon"}) "canonCard": { ... }, // optional: the standing exit
 * as a full card with the // same anatomy (label, thesis, palette, comp, ...); // rendered last and
 * visually subordinate. Without it, // canon stays a quiet footer action. "steer": true, // adds a
 * free-text steer field returned with any answer "followup": true // this round's pick is not
 * terminal: the server // stays open awaiting --update with the next // round (detached mode only),
 * the page shows a // loading hand instead of goodbye, and the // answer carries followup:true so
 * --wait knows // to keep the table. Use it when a decision has // a known second half, e.g.
 * direction first, // then the execution contract. }
 *
 * Options render as large cards: the comp leads when present, with the inspiration image
 * picture-in-picture; a hero alone renders full-bleed; a text-only direction gets its identity from
 * the palette chips and tags. Local image paths are served by this server; nothing is uploaded
 * anywhere.
 *
 * Modes: (default) block until answered; ANSWER on stdout; exit 0. --schema print the canonical
 * payload example and exit. --start for harnesses that cannot leave a shell blocked: daemonize the
 * server, print QUESTION URL + QUESTION KEY, exit immediately. Never auto-opens a browser: the
 * agent routes the URL to the best surface it has (in-app browser first, then the system opener);
 * pass --open to force the system browser instead. --wait --key K [--poll 60] poll for the answer:
 * exit 0 + ANSWER line, exit 3 WAITING (run --wait again), exit 2 server gone, exit 4 PAGE CLOSED
 * (the tab went away without an answer; re-present, reopen the URL, or fall back). --stop --key K
 * kill a daemonized question. --update --key K --payload F deliver the next hand after a re-roll:
 * the live page swaps to loading cards when the user re-rolls, and reloads into this new payload
 * the moment it lands. Always the same key the round started with; a second --start serves a new
 * URL and strands the open tab on a hand that never arrives.
 *
 * --timeout bounds the wait for a page to arrive, never the user's decision: once the page
 * heartbeats, the server lives while the page does, and exits only after --idle-grace seconds
 * (default 600) pass with no beat, wide enough to survive a closed laptop lid mid-decision.
 *
 * Node serve-question.mjs --payload question.json [--timeout 900] [--idle-grace 600] [--no-open]
 * [--port 0]
 */
    ...
    while (Date.now() < deadline) {
        ...
        try {
            ...
            const midDelivery = (() => {
                ...
                return (
                    Boolean(state.claimedAt) && Date.now() - state.claimedAt < NEXT_CLAIM_GRACE_MS
                );
            })();
...
if (hasFlag('start')) {
    ...
    const child = spawn(
        ...
        [
            ...
            '--timeout',
            String(timeoutSec),
            ...(arg('idle-grace') ? ['--idle-grace', arg('idle-grace')] : []),
...
function page(waiting = false) {
    ...
    const waitBudgetMs = waiting
        ? Math.max(0, awaitingNextSince + idleGraceMs - Date.now())
        : idleGraceMs;
...
server.listen(portArg, '127.0.0.1', () => {
    ...
    const lifetime = setInterval(() => {
        if (!server.lastBeatSeen) {
            ...
            if (pending) {
                try {
                    deliveredAt = fs.statSync(pending).mtimeMs;
                } catch {
                    /* nothing delivered */
                }
            }
            if (Date.now() - Math.max(deliveredAt, server.lastClaimAt || 0) > NEXT_CLAIM_GRACE_MS) {
                console.log(
                    'serve-question: the page stopped beating and never came back; exiting'
                );
                process.exit(2);
            }
        }
