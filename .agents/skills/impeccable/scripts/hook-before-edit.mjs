 ...
 *
 * Cursor's stop hook is not consistently dispatched by the headless agent, so this hook checks
 * proposed Write/Edit content before it lands. It only denies writes when the real detector finds
 * an issue in the proposed UI content.
 *
 * Contract: never break a turn accidentally. On malformed input or internal errors, allow the tool
 * and exit 0.
 */
...
function hasFragmentEditContent(input) {
    ...
    return (
        Array.isArray(input.edits) && input.edits.some((edit) => edit && typeof edit === 'object')
    );
}
...
function cursorBlockMessage(findings, filePath, config, cwd, footerMode, reserveChars) {
    ...
    const budget = Math.min(limits.maxChars || DEFAULT_CONFIG.limits.maxChars, CURSOR_DENY_LIMIT);
