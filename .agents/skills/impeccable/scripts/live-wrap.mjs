 ...
 *
 * Usage: node <scripts_path>/live-wrap.mjs --id SESSION_ID --count N --query "hero-combined-left"
 * [--file path]
 *
 * Searches project files for the element matching the query (class name, ID, or text snippet),
 * wraps it with the variant scaffolding, and prints the file path
 *
...
/**
 * Build search query strings in priority order (most specific first). ID is most reliable, then
 * specific class combos, then single classes, then raw query.
 */
...
/**
 * Comment syntax, style mode, and preview strategy all come from the framework registry, keyed on
 * the target file's extension: `.jsx`/`.tsx` author JSX comments, `.astro` needs global-prefixed
 * preview CSS because Astro scopes component styles away from the generated wrappers, `.svelte`
 * gets component preview. See live/frameworks/index.mjs for why extension and not project.
 */
...
/**
 * Search project files for the query string (class name, ID, etc.) Returns the first matching file
 * path, or null.
 *
 * Only `node_modules`, `.git`, and `.impeccable` are skipped outright. dist/build/out are left to
 * the isGeneratedFile guard so the `includeGenerated` second pass can still find the element there
 * and report `generatedMatch`.
 */
...
/**
 * Regex that matches a tag opener on a line. Allows the tag name to be followed by whitespace, `>`,
 * `/`, or end-of-line so that multi-line JSX openers (e.g. `<section\n className="..."\n>`) are
 * recognised.
 */
 ...
 *
 * `query` is a class name, attribute fragment (`class="..."`, `className="..."`, `id="..."`), or a
 * raw text snippet. Because a query can appear on a continuation line of a multi-line tag (e.g. the
 * `className="..."` row of a `<section\n className="..."\n>` JSX tag), we walk backward from the
 * match line to find the actual tag opener. When `tag` is provided, opener candidates must match
 * that tag name.
 */
/**
 * Return the smallest leading-whitespace count across a set of lines, ignoring blank lines (whose
 * indent isn't load-bearing). Used to compute the common base indent of a multi-line picked element
 * so reindenting under the wrapper preserves the relative depth between lines.
 */
...
/**
 * Like findElement, but returns every match. Used for ambiguity detection when the agent passes
 * --text: when the same className appears on multiple sibling elements (a list of cards, repeated
 * section variants, etc.), first-match silently lands on the wrong branch. Returning all matches
 * lets the caller narrow by textContent or fail with a structured ambiguity error.
 */
...
/**
 * Narrow a candidate set to those whose source body matches a meaningful prefix of the picked
 * element's textContent. The compare strips tags and JSX expressions, then checks two whitespace
 * normalizations side-by-side:
 *
 * - Single-space ("hero two second card body")
 * - No-whitespace ("herotwosecondcardbody")
 *
 * Both are needed because `el.textContent` concatenates sibling text without inserting whitespace
 * (e.g. `<h1>Hero Two</h1><p>Second…</p>` reads as `"Hero TwoSecond…"`), while the source has
 * whitespace between tags. If EITHER normalization matches, the candidate keeps. A snippet shorter
 * than 8 chars after stripping is too weak to disambiguate — the caller falls back to first-match.
 */
...
/**
 * Resolve a match line to the real tag opener. If the match line itself opens a tag, return it.
 * Otherwise walk up to 10 lines backward looking for the first tag opener. If `tag` is specified,
 * the opener must match that tag name; an opener with a different tag name aborts the backward walk
 * for this match (we don't jump across element boundaries).
 *
...
/**
 * Starting from a line with an opening tag, find the line with the matching closing tag by counting
 * tag nesting depth.
 */
