 ...
 *
 * - Open the project's live dev/preview URL in the browser (optional, if browser automation
 *   exists)—not `serverPort`; that port is the Impeccable helper for /live.js and /poll
 * - Enter the harness-native poll loop: `node live-poll.mjs`
 *
 * Usage: node live.mjs # Prepare everything, print JSON, exit node live.mjs --help
 */
  ...
  4. Enter the poll loop: node live-poll.mjs`);
    ...
    try {
        ...
        const briefRoots = [roots.appRoot, roots.contextRoot, roots.repoRoot]
            ...
            .filter(
                (dir, i, arr) =>
                    arr.findIndex((other) => path.resolve(other) === path.resolve(dir)) === i
            );
...
/**
 * Drift-heal scan. Walks the project for HTML files under common page-source directories (public/,
 * src/, app/, pages/) and reports any that aren't covered by the resolved inject targets. This is
 * purely advisory — the agent can ignore it, or suggest the user add the orphans to config.files.
 *
 * Skipped if config.files already contains at least one glob pattern covering everything in
 * practice (signaled by the orphan count being 0).
 */
...
/**
 * Same glob-to-regex mapping used by live-inject.mjs. Kept inline here to avoid a circular import
 * (live-inject.mjs already imports nothing from live.mjs). The two must stay in sync.
 */
