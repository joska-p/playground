 ...
 *
 * Serves the browser script (/live.js), the detection overlay (/detect.js), uses Server-Sent Events
 * (SSE) for server→browser push, and HTTP POST for browser→server events. Agent communicates via
 * HTTP long-poll (/poll).
 *
 * Usage: node <scripts_path>/live-server.mjs # start node <scripts_path>/live-server.mjs stop #
 * stop + remove injected live.js tag node <scripts_path>/live-server.mjs stop --keep-inject # stop
 * only node <scripts_path>/live-server.mjs --help
 */
 ...
 *
 * The preflight no longer writes the scaffold into source for source-preview targets (the agent
 * writes wrapper + variants in one atomic edit), so the old scaffold-write full-reload that opened
 * the "stranded at 0/N" race is gone. This recovery stays as defense in depth: any framework reload
 * that drops the agent's variant write + `done` while the browser is mid-reload leaves the new page
 * in GENERATING at 0/N. That resumed page always checkpoints (`browser_resumed`), so a checkpoint
 * claiming "still generating, variants missing" for a session whose generation already completed is
 * direct evidence of the miss. Rebuild the `done` payload from the snapshot so the caller can
 * re-broadcast it; the browser's done handler is idempotent and falls back to injecting variants
 * from source.
 *
 * Keys on the store's monotone `generationCompletedAt`, not `phase` — the behind checkpoint itself
 * regresses `phase` to `generating`, and a browser that misses the redelivered `done` too (another
 * reload) must still trigger redelivery from its next checkpoint.
 */
...
function releasePendingEvent(id, sourceEventType) {
    const entry = state.pendingEvents.find(
        (item) =>
            item.event?.id === id && (!sourceEventType || item.event?.type === sourceEventType)
    );
        ...
        if (p === '/events' && req.method === 'POST') {
            ...
            req.on('end', () => {
                ...
                if (msg.type === 'manual_edit_apply') {
                    ...
                    res.end(
                        JSON.stringify({
                            error: 'manual_edit_apply is disabled; use /manual-edit-stash then /manual-edit-commit'
                        })
                    );
                    return;
                }
...
/**
 * A previous run that died without its shutdown hook leaves preview component dirs behind. Drop the
 * ones whose session the store no longer considers active; anything still active is mid-generation
