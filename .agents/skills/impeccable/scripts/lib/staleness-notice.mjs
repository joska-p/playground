 ...
 *
 * The boot path already carries PRODUCT.md, DESIGN.md, a surface brief, RESOLVED_CONTEXT, the
 * detector fallback, native platform references, and the update directive. An unthrottled staleness
 * block would push real context out of attention and train the agent to open every session with
 * housekeeping, so the rules here are deliberately strict:
 *
 * - One directive for the whole set, never one per finding.
 * - A 'mention' or 'route' finding surfaces at most once a week per project, mirroring the update
 *   check's anti-nag window. A finding the user has already declined to act on must not reappear
 *   tomorrow.
 * - 'auto' findings are not throttled and are not shown to the user. They are migrations the next
 *   write performs anyway, so the agent needs the note every session until the write happens, and
 *   the user needs it never.
 *
 * State lives in the user's home dir alongside the update cache rather than in the project, so no
 * gitignore entry is owed and a clone does not inherit someone else's dismissals.
 */
...
/**
 * Drop project entries whose newest stamp has aged past the renotify window. They would be
 * re-notified on the next boot anyway, so keeping them only lets the file accumulate one entry per
 * directory Impeccable has ever booted in (scratch dirs and test fixtures included).
 */
 ...
 * .impeccable/config.json. Local config overrides shared, matching how updateCheck resolves.
 */
...
/**
 * Drop findings already surfaced for this project inside the renotify window, and stamp the ones
 * that survive. 'auto' findings pass through untouched and unstamped: they are for the agent, not
 * the user, and repeat until fixed.
 */
export function filterFreshFindings(findings, { projectRoot, now = Date.now() } = {}) {
    ...
    const seen =
        cache.projects[key] && typeof cache.projects[key] === 'object' ? cache.projects[key] : {};
