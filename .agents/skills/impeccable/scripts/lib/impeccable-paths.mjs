...
/**
 * Session IDs become path segments (journals, snapshots, accept receipts, preview manifests,
 * generated component dirs). They arrive from CLI `--id` arguments and HTTP payloads, so anything
 * containing a separator or `..` must be rejected before it reaches path.join, which would happily
 * escape `.impeccable/live/`. Real IDs are 8 hex chars; the tests use short slugs.
 */
