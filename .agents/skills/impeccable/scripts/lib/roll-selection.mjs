...
function challengerTickets(pool) {
    return pool.flatMap((concept) => {
        ...
        return Array.from({ length: ticketsForRating(concept.review?.rating) }, (_, ticket) => ({
            concept,
            ticket
        }));
    });
...
function compositionTickets(pool) {
    return pool.flatMap((composition) =>
        Array.from({ length: ticketsForRating(composition.review?.rating) }, (_, ticket) => ({
            composition,
            ticket
        }))
    );
...
/**
 * Six challengers, two per translation tier, from an explicit approved pool. Drive with
 * runSyncSelection or runAsyncSelection.
 *
...
/**
 * Three identity-free composition inputs from an explicit approved pool. Drive with
 * runSyncSelection or runAsyncSelection.
 *
 * One input was too weak a counterweight to a model's habitual page skeleton: it became a single
 * optional flourish beside six identity challengers rather than a real search over composition.
 * Distinct composition families are preferred so a roll tests materially different hierarchy,
 * sequence, and interaction laws. Cross-mode fallback would make the input misleading, so an absent
 * mode returns nothing rather than borrowing. Re-rolls exclude every earlier set until the pool
 * runs out.
 *
 * @param {object} options
 * @param {'direction' | 'surface'} options.scope
