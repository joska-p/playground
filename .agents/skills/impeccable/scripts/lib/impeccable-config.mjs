 ...
 *
 * The CLI (published to npm) and the skill scripts (bundled into the install) live in separate
 * trees and cannot share runtime code, so this duplicates a small slice of
 * skill/scripts/hook-lib.mjs — the config-path layout, detector ignore semantics, and the
 * `.git/info/exclude` handling. Keep the schema, ignore filtering, and exclude marker in sync if
 * either side changes.
 *
 * Schema (config.json shared / config.local.json gitignored, per-developer): { "detector": {
...
/**
 * Detector filters shared by `npx impeccable detect` and the design hook. `hook.enabled` remains
 * hook lifecycle state; manual CLI scans still run when the hook is disabled, but they honor the
 * same ignore rules and design-system toggle.
 */
...
function parseIgnoreColor(value) {
    ...
    if (rgb) {
        ...
        const a =
            parts[3] === undefined ? 1 : parseColorChannel(parts[3], COLOR_CHANNEL_FORMATS.alpha);
...
/**
 * The recorded design-hook decision: 'accepted' | 'declined' | undefined. config.local.json
 * (per-developer) overrides config.json.
 */
...
/**
 * Persist the per-developer decision to config.local.json, preserving any sibling keys, and ensure
...
/**
 * Add config.local.json to `.git/info/exclude` so a developer's decision is never committed.
 * Idempotent via marker comments. Best-effort; returns false when there is no resolvable git dir.
 */
