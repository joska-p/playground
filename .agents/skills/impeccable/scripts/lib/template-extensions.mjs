 ...
 *
 * Before this module the answer was spelled out separately in hook-lib.mjs (`detector.extensions`
 * config, issue #316) and in live-wrap.mjs / live-accept.mjs (a hardcoded `EXTENSIONS` array,
 * duplicated verbatim in both). The lists drifted: the hook learned configurable server-template
 * extensions while Live kept its six frontend defaults, so a Phoenix project got design findings on
 * `.heex` files but `Session markers not found` on Accept (#374).
 *
 * Extensions are matched against the END OF THE FILENAME, not `path.extname`, so double extensions
 * like `.blade.php`, `.html.erb`, and `.html.heex` work.
 */
 ...
 *
 * Elixir's `.ex` is here because Phoenix function components put `~H"""` templates directly in
 * `lib/**\/*.ex`; `.heex` and `.eex` cover standalone templates. `.exs` is deliberately absent:
 * those are Elixir _scripts_ (`mix.exs`, `config/*.exs`, tests) and never hold markup, so including
 * them only gives the wrap query a chance to match build config by accident.
 */
 ...
 *
 * Accepts `{ ext, engine }` objects (engine 'html' | 'text', default 'html' — the common case for
 * server-side templates) or bare strings as shorthand.
 */
 ...
 *
 * Suffix matching rather than `path.extname` equality, so a configured `.html.erb` matches
 * `show.html.erb` (whose extname is only `.erb`). The `name.length > ext.length` guard keeps a file
 * literally named `.heex` from counting as a template.
 */
 ...
 *
 * Reading `detector.extensions` here is the point: a user who taught the design hook about
 * `.blade.php` should not have to teach Live separately. Config parsing is intentionally minimal
 * (own the shape, not the whole hook config) so this module stays importable from the Live CLI
 * without pulling in hook-lib.mjs.
 */
