/**
 * Tier 2 staleness checks: the ones that cost too much to run on every session boot. Shelling out
 * to git, walking workspaces, resolving hook script paths, and validating ignore lists against the
 * live rule registry all belong here.
 *
 * The boot tier answers "did an older Impeccable write this". This tier also asks "does it still
 * describe the code", which no file comparison can settle on its own. Where the answer needs
 * judgment, the finding reports a measured proxy and says it is a proxy. It never claims a document
 * is wrong because a number is large.
 *
...
/**
 * How much UI work has landed since DESIGN.md was last touched, measured in commits to the visual
 * source directories. A proxy, and reported as one: a large number means the document is worth
 * re-reading, not that it is wrong. Silent outside a git repo, on an untracked DESIGN.md, and when
 * the count is small enough to be ordinary maintenance.
 */
...
/**
 * Canonical DESIGN.md sections that carry nothing. Distinct from truth drift: a section can be
 * absent because it never applied, so this is reported as a documentation gap for a human to judge,
 * never as an error.
 */
...
/**
 * Ignore entries that no longer match anything: rule ids the engine dropped or renamed, and file
 * paths that are gone. Both read as working suppressions until someone checks, and a dead rule
 * ignore also hides that the rule left.
 */
...
/**
 * A hook whose script path does not resolve is a silent no-op, and the user believes the project is
 * covered. Also catches the contradiction of an installed manifest against `hook.enabled: false`.
 */
...
/**
 * Per-workspace context, plus the case worth acting on: a workspace with native build files
 * inheriting a repo-root PRODUCT.md that says web. Each such app gets web guidance and never loads
 * the native references, and nothing at boot reports it because the root record parses cleanly.
 *
...
/**
 * Rule ids from the bundled detector, or null when it cannot be resolved (a partial install, or a
 * harness that ships the skill without the engine). Null means "cannot check", which the
 * ignore-rule check treats as skip rather than as every id being unknown.
 */
