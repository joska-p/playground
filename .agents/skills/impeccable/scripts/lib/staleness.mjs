/**
 * Staleness detection for Impeccable's own project artifacts: PRODUCT.md, DESIGN.md and its
 * `.impeccable/design.json` sidecar, `.impeccable/config.json`, and persisted surface briefs.
 *
 * Three kinds of drift live under "out of date", and they want different handling:
 *
 * 1. Tool version drift. The installed skill is older than the published one. Owned by
 *    computeUpdateDirective in context.mjs, not by this module.
 * 2. Schema drift. An artifact was written by an older Impeccable: fields it no longer reads, fields
 *    it now expects, files in retired locations. Deterministic, and mostly fixable without asking
 *    anyone.
 * 3. Truth drift. The code moved on and the document no longer describes it. Not mechanical.
 *    `document` and `init` own the rewrite; the most this module does is measure a proxy and name
 *    it as a proxy.
 *
 * Two tiers, because the boot path runs on every session:
 *
 * Tier 1 (collectBootFindings) spends only what a boot already spends. It parses markdown
 * context.mjs has in memory, stats a bounded set of paths, and reads the two small JSON files the
 * boot reads anyway. No directory walks, no git, no cross-workspace sweep.
 *
 * Tier 2 (the doctor pass) is on demand and may walk, shell out to git, and compare declared tokens
 * against real CSS.
 *
 * Findings are data, not prose, so both tiers and the JSON output render the same set. Severity
 * says what should happen, not how bad it is:
 *
 * 'auto' fix it silently the next time that file is written anyway 'mention' state it once, offer
 * the fix, carry on with the user's task 'route' needs a specific command, so name the command and
 * the gap
 */
...
/**
 * Every location a design sidecar may live, canonical first. Pure so that both impeccable-paths
 * (which resolves the project root) and context.mjs (which cannot import impeccable-paths without a
 * cycle) share one definition of where the retired locations are.
 */
...
/** Pure: schema drift visible in a PRODUCT.md body. `productPath` is used for reporting only. */
...
/**
 * A project that resolves to web while carrying native build files. Bounded: a handful of stats
 * plus one package.json read at the project root.
 */
...
/**
 * Sidecar drift: retired location, schema version behind, or older than the DESIGN.md it extends.
 * Costs three stats and one small JSON read.
 *
 * `sidecarCandidates` comes from impeccable-paths' resolver so this module stays out of the
 * business of knowing where sidecars may live; the first entry is the canonical location.
 */
...
/**
 * Unrecognized keys in the shared and local configs. A key nothing reads is indistinguishable from
 * a working setting until someone checks, which is how a singular `ignoreRule` silences nothing for
 * months.
 */
...
/**
 * No recorded build-path preference on a project that plainly does visual direction work. Not drift
 * in the usual sense: the setting is newer than the project, so every project that predates it
 * lands here at once. That is why it is gated twice, on a product record and on evidence of the
 * work the setting governs, and why it says the choice rather than assuming a harness can make it.
 * Image generation is the real precondition and this module cannot see it: a harness-native image
 * tool leaves no trace on disk, so the finding hands the question to the one reader that knows.
 */
...
/**
 * A brief whose primary target no longer exists still resolves and still gets injected as authority
 * for a surface that is gone. Route and URL targets have no file to check and are skipped.
 */
...
/**
 * `projectRoots` globs that match no directory. When every pattern misses, candidate discovery
 * returns nothing, the repo root silently becomes the active project, and no other signal fires.
 *
 * Takes the candidate list rather than computing it: the boot path has already paid for that walk,
 * and this module must not pay for it twice.
 */
...
/**
 * Workspaces that inherit the repo-root PRODUCT.md. Inheritance is a feature, not a defect, so this
 * is reported as information for the doctor pass rather than emitted at boot: the judgment call is
 * whether the inherited record actually describes that app.
 */
...
/**
 * Everything a boot can afford, grouped by artifact so deeper reports can interleave their own
 * checks without rebuilding this policy. `ctx` is the loadContext result; `extras` carries values
 * the caller already computed so nothing is recomputed here.
 */
