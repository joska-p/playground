...
if (IS_BROWSER) {
  ...
  `;
    ...
    function resolveObjectImageRect(containerRect, image, style) {
        ...
        return {
            left:
                containerRect.left + parsePositionToken(xToken, containerRect.width, paintedWidth),
            top:
                containerRect.top + parsePositionToken(yToken, containerRect.height, paintedHeight),
            width: paintedWidth,
            height: paintedHeight,
            intrinsicWidth,
            intrinsicHeight
    ...
    async function analyzeVisualContrastCandidate(candidate) {
        ...
        const pick = (pct) =>
            ratios[
                Math.min(ratios.length - 1, Math.max(0, Math.floor((pct / 100) * ratios.length)))
            ];
