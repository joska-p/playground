...
/**
 * Strip HTML to plain text — drops script/style/comments/tags so content-text analyzers don't
 * false-positive on code or CSS.
 */
...
/**
 * Blank JavaScript comments without moving any following source. Regex findings keep their original
 * line numbers, while prose examples inside comments cannot masquerade as rendered markup.
 */
...
function blankHtmlAndCssCommentsOutsideScripts(text) {
    ...
    let match;
    while ((match = re.exec(text)) !== null) {
        output += blankCssLineCommentsInStyleBlocks(
            stripCssComments(blankHtmlComments(text.slice(lastIndex, match.index)))
        );
        output += match[0];
        lastIndex = re.lastIndex;
    }
    return (
        output +
        blankCssLineCommentsInStyleBlocks(
            stripCssComments(blankHtmlComments(text.slice(lastIndex)))
        )
    );
}
 ...
 *
 * Whitespace inside parens does not separate tokens: `rgb(0 0 0)` and `var(--x, 4px)` are each a
 * single value, and splitting them on spaces would read their innards as separate lengths.
 */
 ...
 *
 * Shared/color.mjs's isNeutralColor only parses the computed function forms a browser or jsdom
 * emits (rgb/oklch/lab/...) and deliberately reports every other spelling as chromatic so an
 * unknown format is never silently skipped. That default is wrong for authored CSS, where `#000`
 * and `black` are the normal spellings: calling it directly reports a plain black hairline as a
...
const REGEX_MATCHERS = [
    ...
    {
        id: 'overused-font',
        regex: /font-family\s*:\s*['"]?(Inter|Roboto|Open Sans|Lato|Montserrat|Arial|Helvetica|Fraunces|Geist Sans|Geist Mono|Geist|Mona Sans|Plus Jakarta Sans|Space Grotesk|Recoleta|Instrument Sans|Instrument Serif)\b/gi,
        test: () => true,
        fmt: (m) => m[0]
    },
    {
...
/**
 * Blank out comment bodies while preserving every byte offset (and therefore every line number) so
 * commented-out CSS is not scanned as live rules.
 */
...
/**
 * Page-level analyzers that scan rendered text content (em-dash use, buzzword phrases, aphoristic
 * cadence). These are detector-agnostic — they work on any HTML/text source and don't need a parsed
 * DOM. Exported so detectHtml can call them for `.html` files (which otherwise skip the regex
 * engine).
 */
...
function detectText(content, filePath, options = {}) {
    ...
    findings.push(
        ...runRegexMatchers(lines, filePath, 0, STYLESHEET_EXTS.has(ext) || null, {
            profile,
            phase: 'source'
        })
    );
