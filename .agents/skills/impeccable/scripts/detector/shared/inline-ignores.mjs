/**
 * Inline, in-file ignore directives — eslint-disable-style waivers that live at the point they
 * apply and travel with the artifact instead of (or alongside) an ignore in
 * `.impeccable/config.json`.
 *
 * A config ignore is the right default for repo-wide policy. This complements it for the one case
 * config can't cover: a waiver that belongs to a single file and needs to follow that file when it
 * leaves the repo — a generated/exported standalone document, an emailed HTML file, a snippet
 * scanned out of context.
 *
 * Comment-syntax-agnostic: the directive is a raw token matched anywhere on a line, so the same
 * marker works across every comment style impeccable scans — `//`, `/* *\/`, `<!-- -->`, `#`, `{/*
 * *\/}`, `{# #}`. Trailing comment closers are stripped before the rule list is parsed.
 *
 * Syntax (reason optional; eslint `--` or biome `:` separator):
 *
 * Impeccable-disable <rule>[, <rule>...] [-- reason] whole file impeccable-disable-line <rule>...
 * [-- reason] the same line impeccable-disable-next-line <rule>... [-- reason] the following line
 * impeccable-disable bare / `*` = every rule
 *
 * Examples:  <!-- impeccable-disable overused-font -- exported brand doc, font is first-party -->
 *
 * .brand { font-family: Inter; } /* impeccable-disable-line overused-font */ //
 * impeccable-disable-next-line bounce-easing: intentional playful affordance
 *
 * Behavior is suppression, for parity with config ignores: a matched directive drops the finding.
 * The inline reason is self-documenting in the diff; it is not required and is discarded at scan
 * time (only used here to keep reason words out of the parsed rule list).
 */
 ...
 *
 * Returns sets keyed by the 1-based line the directive _targets_ so matching is a direct lookup:
 *
...
/**
 * Drop findings waived by an inline directive in the same file's source text. Findings without a
 * usable line number (e.g. static-HTML page-level findings) are only matched by whole-file
 * directives — which is the standalone-document case this primitive exists for.
 */
