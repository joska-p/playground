/**
 * Anti-Pattern Browser Detector for Impeccable Copyright (c) 2026 Paul Bakaus
 * SPDX-License-Identifier: Apache-2.0
 *
 * GENERATED -- do not edit. Source: cli/engine/browser/injected/index.mjs Rebuild: node
 * scripts/build-browser-detector.js
 *
 * Usage: <script src="detect-antipatterns-browser.js"></script> Re-scan: window.impeccableScan()
 */
(function () {
    ...
    function colorFunctionToRgb(space, c1, c2, c3) {
        switch (space) {
            ...
            case 'display-p3': {
                const [R, G, B] = [
                    decodeSrgbChannel(c1),
                    decodeSrgbChannel(c2),
                    decodeSrgbChannel(c3)
                ];
                return linearSrgbToColor(
        ...
        while (cur && cur.nodeType === 1) {
            const attr =
                typeof cur.getAttribute === 'function'
                    ? cur.getAttribute('data-impeccable-ignore')
                    : null;
            if (attr != null) {
    ...
    function checkColors(opts) {
        ...
        if (classList) {
            const classStr =
                typeof classList === 'string' ? classList : Array.from(classList).join(' ');
    ...
    function findShadowColor(layer) {
        ...
        if (fn)
            return { color: parseAnyColor(fn[0]), start: fn.index, end: fn.index + fn[0].length };
        ...
        while ((m = wordRe.exec(layer)) !== null) {
            ...
            if (named)
                return { color: { ...named, a: 1 }, start: m.index, end: m.index + m[0].length };
        }
        return null;
    }
    ...
    function scanCssTextForPseudoStripe(rawContent) {
        ...
        while ((m = ruleRe.exec(content)) !== null) {
            ...
            if (!bg || /^(?:none|transparent|inherit|initial|unset|currentcolor)$/i.test(bg))
                continue;
    ...
    function scanHtmlForShapeAssembledIllustration(html) {
        ...
        while ((m = svgRe.exec(html)) !== null) {
            ...
            for (const fm of block.matchAll(/\bfill\s*[:=]\s*["']?\s*([^"';>}\s]+)/gi)) {
                ...
                if (!paint || ['none', 'transparent', 'currentcolor', 'inherit'].includes(paint))
                    continue;
                fills.add(paint);
            }
        ...
        while (current && current.nodeType === 1) {
            ...
            if (hasGradientOrUrl) {
                ...
                if (urlBeneath) {
                    ...
                    const provablyOpaque =
                        topStops.length > 0 && topStops.every((s) => (s.a ?? 1) >= 0.99);
    ...
    function checkElementBorders(tag, style, overrides, resolvedRadius, el = null) {
        ...
        const radius =
            resolvedRadius != null ? resolvedRadius : parseFloat(style.borderRadius) || 0;
    ...
    function checkElementColors(el, style, tag, window, customPropMap, hasAnchorInheritRule) {
        ...
        for (
            let cur = el;
            cur && cur.nodeType === 1 && effOpacity > 0.02;
            cur = cur.parentElement
        ) {
    ...
    function checkElementIconTile(el, tag, window) {
        ...
        if (iconChild) {
            ...
            iconWidth =
                parseFloat(iconStyle.width) || parseFloat(iconChild.getAttribute('width')) || 0;
        }
    ...
    function checkElementBlinkingCursorDOM(el) {
        const tag = el.tagName.toLowerCase();
        if (['input', 'textarea', 'select', 'img', 'svg', 'script', 'style'].includes(tag))
            return [];
        const style = getComputedStyle(el);
        ...
        for (const victim of textEls) {
            ...
            for (let i = 0; i < cols; i++) {
                ...
                for (let j = 0; j < rows; j++) {
                    ...
                    if (isFloated(topCs) || isMarqueeish(top, topCs) || isPinnedOverlay(top))
                        continue;
  ...
  `;
        ...
        function collectVisualContrastReasons(el, style) {
            ...
            while (current && current.nodeType === 1) {
                ...
                const solidBg =
                    parseRgb(currentStyle.backgroundColor) ||
                    parseAnyColor(currentStyle.backgroundColor);
            ...
            if (sampleRect && document.elementsFromPoint) {
                const points = [
                    ...
                    [
                        sampleRect.left +
                            Math.min(sampleRect.width - 1, Math.max(1, sampleRect.width * 0.25)),
                        sampleRect.top + sampleRect.height / 2
                    ],
                    [
                        sampleRect.left +
                            Math.min(sampleRect.width - 1, Math.max(1, sampleRect.width * 0.75)),
                        sampleRect.top + sampleRect.height / 2
                    ]
                ];
            ...
            if (img.currentSrc || img.src) {
                ...
                if (loaded) {
                    const loadedRect = {
                        ...
                        intrinsicHeight:
                            loaded.naturalHeight || loaded.height || paintedRect.intrinsicHeight
                    };
        ...
        async function analyzeVisualContrastCandidate(candidate) {
            ...
            const style = getComputedStyle(el);
            const textColor =
                parseRgb(style.color) || parseAnyColor(style.color) || candidate.textColor;
            if (!textColor)
                return {
                    ...candidate,
                    status: 'unresolved',
