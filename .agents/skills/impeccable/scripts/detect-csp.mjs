/**
 * Scan a project tree for Content-Security-Policy signals and classify the shape so the agent knows
 * which patch template to propose.
 *
 * Used at first-time `live.mjs` setup. Mechanical (grep-based) — no network, no dev server, no JS
 * evaluation. The classification drives a user-facing consent prompt; the agent does the actual
 * patch writing.
 *
 * Shapes are named by patch mechanism, not framework origin:
 *
 * - "append-arrays": CSP defined as structured directive arrays. Patch appends a dev-only localhost
 *   entry. Covers: - Monorepo helpers with additional*Src options (e.g. createBaseNextConfig for
 *   Next) - SvelteKit kit.csp.directives - nuxt-security module's contentSecurityPolicy
 * - "append-string": CSP built as a literal value string. Patch splices a dev-only token into
 *   script-src and connect-src. Covers: - Inline Next.js headers() with CSP string - Nuxt
 *   routeRules / nitro.routeRules CSP headers
 * - "middleware": CSP set dynamically in middleware.{ts,js}. Detected but not auto-patched in v1.
 * - "meta-tag": <meta http-equiv="Content-Security-Policy"> in layout files. Detected but not
 *   auto-patched in v1.
