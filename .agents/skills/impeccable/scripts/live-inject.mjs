/**
 * CLI helper: insert/remove the live variant mode script tag in the project's main HTML entry
 * point.
 *
 * On first live run, the agent generates `.impeccable/live/config.json` with the project's
 * insertion target (framework-specific). On every subsequent run, this script handles insert/remove
 * deterministically with zero LLM involvement.
 *
 * Framework knowledge lives in `live/frameworks/` — detection order, adapters, the generic tag
 * strategy, and the per-extension authoring traits live-wrap reads. This file is the CLI around it:
 * resolve config, resolve the framework, heal orphaned artifacts, apply or remove, record the
 * journal.
 *
 * Usage: node live-inject.mjs --port PORT [--token TOKEN] # Insert the live script tag node
 * live-inject.mjs --remove # Remove the live script tag node live-inject.mjs --check # Check
 * whether live config exists
 *
 * When --token is supplied, it is appended to the /live.js src as `?token=...` so the server's
 * token-gated /live.js handler will serve the bundle. Omitting the token yields a bare `/live.js`
 * src (legacy behavior; the server returns 401 for it under the current gate).
 */
...
/**
 * Hard-excluded directory patterns. These are NEVER user-facing pages and matching them would
 * silently inject tracking scripts into third-party code. The user cannot turn these off via config
 * — they are the floor.
 */
  ...
  { ok, file, inserted|removed, config? }`);
    ...
    if (args.includes('--check')) {
        ...
        if (!fs.existsSync(CONFIG_PATH_GET())) {
            console.log(
                JSON.stringify({ ok: false, error: 'config_missing', path: CONFIG_PATH_GET() })
            );
            process.exit(0);
        }
        let cfg;
    ...
    if (args.includes('--remove')) {
        ...
        clearInjectJournal(cwd);
        console.log(
            JSON.stringify({ ok: true, results, healed: healed.length ? healed : undefined })
        );
        return;
    }
...
/**
 * Expand config.files (which may contain glob patterns) into a literal list of existing file paths
 * relative to rootDir. Literal entries pass through; glob patterns are expanded via fs.globSync.
 * HARD_EXCLUDES and config.exclude are applied as filters. Duplicates are removed. Order is
...
/**
 * Convert a glob pattern to a RegExp. Supports: ** → any number of path segments (including zero)
 *
 * - → any chars except `/` ? → any single char except `/` Paths are normalized to forward slashes
 *   before matching.
 */
