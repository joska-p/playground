 ...
 *
 * Usage: node live-accept.mjs --id SESSION_ID --discard node live-accept.mjs --id SESSION_ID
 * --variant N
 *
 * For discard: removes the entire variant wrapper and restores the original. For accept: replaces
 * the wrapper with the chosen variant's content. If the session had a colocated <style> block, it's
 * preserved with carbonize markers for a background agent to integrate into the project's CSS.
 *
 ...
 *
 * Live/completion.mjs only classifies a result as `error` when it carries `mode: 'error'`; anything
 * else unhandled falls through to `agent_done` with a successful ack, and reference/live.md then
 * tells the agent to finish the edit by hand. That is right for the documented fallback paths and
 * wrong here: a `source_locked` contention needs a retry (hand-editing races the publisher holding
 * the lock), and a crash needs surfacing, not a hand-applied guess.
 */
 ...
 *
 * OperationFailure only covers results built from a _thrown_ error. The accept implementations also
 * return `{handled: false, error}` for their own checks (variant missing, template empty, original
 * text ambiguous), and those arrived without `mode`, so completion.mjs classified them as
 * agent_done and reference/live.md routed the agent to "read file, find markers, edit".
 *
 * That handoff only makes sense for a plain wrapper session, which is the one shape with markers in
 * the user's source to edit. Component and isolated artifact previews keep the source clean until
 * Accept, so there is nothing to hand-edit and an unhandled result is always a failure.
 * `previewMode` is exactly that discriminator: only the preview branches set it.
 */
...
/**
 * After a variant accept rewrites one wrapper, drop only buffer ops whose text appeared inside that
 * wrapper's original block. The previous file-wide scrub dropped unrelated staged edits from other
 * components/files whenever their originalText wasn't present in the just-accepted file.
 *
 * Match both originalText and newText because live-wrap rewrites the original preview block to
 * reflect pending manual edits before variants are generated.
 */
...
/**
 * Build carbonize stitch-in lines. JSX targets occupy a single child slot (ternary branch, return
 * value, etc.) — the same constraint as live-wrap. When isJsx, tuck markers + <style> + variant
 * wrapper inside one outer<div data-impeccable-carbonize> so the slot keeps a single root node.
 */
...
/**
 * Find the start/end marker lines for a session. Returns { start, end } (0-indexed line numbers) or
 * null.
 */
...
/**
 * Compute the line range to REPLACE (vs. just the marker range to extract from). For JSX/TSX
 * wrappers, live-wrap places the marker comments INSIDE the `<div data-impeccable-variants="ID">`
 * outer wrapper so the picked element's JSX slot keeps a single child — a Fragment `<></>` would
 * have solved the multi-sibling case but failed inside `asChild` / cloneElement parents with
 * "Invalid prop supplied to React.Fragment".
 *
 * That means the marker block is enclosed by the wrapper `<div>` opener (with
 * `data-impeccable-variants="ID"`) and its matching `</div>`. We walk back to the opener and
 * forward to the closer so accept/discard remove the entire scaffold, not just the inner markers.
 *
 * Marker lines themselves stay where they were so extractOriginal / extractVariant / extractCss
 * continue to walk the same range.
 */
...
/**
 * Join wrapper lines into a single string with `<style>` elements removed so marker matching and
 * div-depth tracking aren't confused by:
 *
 * - CSS `@scope ([data-impeccable-variant="N"])` strings that look like the HTML marker we're
 *   searching for
...
/**
 * Find the inner content of `<TAG ...attrMatch...>…</TAG>` inside `text`, handling nested same-tag
 * elements via depth counting. `attrMatch` is a regex source fragment that must appear inside the
 * opener tag. Returns the inner string (may be empty), or null if not found.
 */
...
/** Extract the original element content from within the variant wrapper. Returns an array of lines. */
...
/**
 * Extract a specific variant's inner content (stripping the wrapper div). Returns an array of
 * lines, or null if not found.
 */
...
/**
 * Extract the colocated <style> block content (between the style tags). Returns an array of CSS
 * lines, or null if no style block found.
 *
 ...
 * 3. Multi-line: `<style>` on one line, `</style>` on a later line — return the lines between them.
 */
...
/**
 * Strip a JSX template-literal wrap (`{` … `}`) from CSS extracted out of a `<style>` element in a
 * JSX/TSX file. The agent may write the wrap with `{` and `}` directly attached to the `<style>`
 * tags, on their own lines, or attached to the first/last CSS lines — all three are JSX-legal.
 *
 * Stripping is required because handleAccept re-wraps the CSS itself when carbonizing. Without
 * this, two consecutive accepts (or a previously- accepted variants block being carbonized) would
 * produce nested `{` `{` … `}` `}`, which oxc rejects with "Expected `}` but found `@`".
 */
...
/**
 * De-indent content that was indented by live-wrap.mjs. The wrap script adds `indent + ' '` (4
 * extra spaces) to each line. We restore to just `indent` level.
 */
...
/**
 * Accept also skips `dist` / `build` outright, where wrap descends into them so its
 * `includeGenerated` second pass can report a `generatedMatch`. Accept has no such pass: a marker
 * found in build output is only ever a stale copy of the marker in source.
 */
