...
function promptOf(imagePath) {
    ...
    if (prompt == null && fs.existsSync(`${imagePath}.json`)) {
        try {
            prompt = JSON.parse(fs.readFileSync(`${imagePath}.json`, 'utf8')).prompt ?? null;
        } catch {
            /* stays null */
        }
    }
...
if (scanMode) {
    ...
    if (targets.length === 0) {
        console.error('embed-prompt: --scan needs at least one directory');
        process.exit(1);
    }
    ...
    for (const target of targets) {
        if (!fs.existsSync(target)) {
            console.error(`embed-prompt: no such path ${target}`);
            process.exit(1);
        }
        walk(target, true);
    }
    ...
    for (const raster of rasters) {
        if (promptOf(raster) == null) {
            console.log(`MISSING: ${raster}`);
            missing++;
        }
    }
    console.log(
        `SCAN: ${rasters.length} raster${rasters.length === 1 ? '' : 's'}, ${missing} missing`
    );
