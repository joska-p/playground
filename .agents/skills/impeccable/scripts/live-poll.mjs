 ...
 *
 * Usage: node <scripts_path>/live-poll.mjs # Block until browser event, print JSON node
 * <scripts_path>/live-poll.mjs --stream # Experimental: keep polling; one JSON line per event node
 * <scripts_path>/live-poll.mjs --timeout=600000 # Custom timeout (ms); default is long-poll
 * friendly node <scripts_path>/live-poll.mjs --reply <id> done # Reply "done" to event <id> node
 * <scripts_path>/live-poll.mjs --reply <id> error "msg" # Reply with error
 */
...
/**
 * Parse `--reply <id> <status> [--file path] [--data '<json>'] [message]` argv into a reply object.
 * Returns null when `--reply` is absent. Throws (code INVALID_REPLY_ARGS) when the reply shape is
 * missing its event id/status and INVALID_DATA_JSON when `--data` is present but not valid JSON.
 */
export function parseReplyArgs(args) {
    ...
    const message =
        args.find(
            (a, i) =>
                i > replyIdx + 2 && !a.startsWith('--') && i !== fileIdx + 1 && i !== dataIdx + 1
        ) || undefined;
