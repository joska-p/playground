 ...
 *
 * The browser Save path stages edits. Apply copy edits calls live-commit-manual-edits.mjs, which
 * builds a page-scoped batch and uses this helper to ask Codex/Claude to edit true source files.
 */
...
export async function runCopyEditBatchAgent(batch, opts = {}) {
    ...
    const provider =
        opts.provider || chooseCopyEditAgent({ env, chatAvailable: opts.chatAvailable });
    if (provider === 'mock') {
...
/**
 * Build a diagnostic error message explaining why no AI runner is usable. Splits the previous
 * "Install/authenticate Codex or Claude" lump into a per-provider summary so the user knows exactly
 * which step unblocks them.
 */
...
/**
 * Pull a human-readable failure reason out of a subprocess's stdout when the process exited
 * non-zero. Recognizes:
 *
 * - Claude CLI `--output-format json` errors: {"is_error": true, "result": "Not logged in · Please
 *   run /login", ...}
 * - Generic JSON payloads with `message` or `error` strings.
 * - The last non-empty line of unstructured output. Returns null when nothing meaningful surfaces, so
 *   the caller can fall back to its existing "X exited with N" message.
 */
...
/**
 * Pre-flight a CLI provider with a trivial prompt and report whether it can actually do work.
 * Cached per process so the `auto` branch of chooseCopyEditAgent only pays the cost once per server
 * boot.
 *
 * For claude we run the same `--print --output-format json` invocation we use for real batches; an
 * unauthenticated CLI fails in ~36 ms with { is_error: true, result: "Not logged in · ..." }. For
 * codex we only confirm the binary exists — `codex exec` always burns a real LLM call, so checking
 * auth without spending tokens is not possible here; if the user has codex installed but unauthed,
 * the runtime error from runCodex (now improved by extractRunnerErrorMessage) will surface
 * clearly.
 */
