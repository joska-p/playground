#!/usr/bin/env node
/**
 * Brand-seed picker. Returns one OKLCH seed color + the mood it most naturally evokes, and teaches
 * the model how to compose a full palette around it.
 *
 * The seed is the brand's anchor color. The 5-role palette (bg, surface, ink, accent, muted) is
 * composed by the caller at runtime using their judgment + the brief (PRODUCT.md / DESIGN.md / user
 * prompt), NOT picked from a frozen 4-color preset.
 *
 * Why: 4-color frozen palettes drift toward safe defaults (warm-cream bg, complementary accent on
 * near-white) regardless of brief. A single seed + the model's own composition lets the same seed
 * produce a dark-mode jazz club or a light-mode hospitality brand depending on what the brief calls
 * for. Tested empirically against curated 4-color palettes; seed approach wins on mood-fit in 3 of
 * 5 cases and ties on the rest.
 *
 * Usage: node scripts/palette.mjs # pick at random node scripts/palette.mjs --id seed-021 # pick a
 * specific seed node scripts/palette.mjs --from <key> # hash <key> to a seed (deterministic)
 *
 * Env vars: IMPECCABLE_PALETTE_SEED — same as --from; useful for the eval harness to make runs
 * reproducible.
 */
