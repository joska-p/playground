#!/usr/bin/env node
/**
 * The Impeccable hooks command manages the design hook runtime via the `hook` key and shared
 * detector ignores via the `detector` key in .impeccable/config.json /
 * .impeccable/config.local.json.
 *
 * Usage: node hook-admin.mjs status # print current state node hook-admin.mjs on # set enabled:
 * true node hook-admin.mjs off # set enabled: false node hook-admin.mjs ignore-rule <rule-id> #
 * append to ignoreRules node hook-admin.mjs ignore-rule overused-font --all-values node
 * hook-admin.mjs ignore-file <glob> [--shared|--local] # append to ignoreFiles node hook-admin.mjs
 * ignore-value <rule> <value> # append to shared ignoreValues node hook-admin.mjs ignore-value
 * <rule> <value> --local node hook-admin.mjs ignore-value <rule> "_" --file <glob> # rule off in
 * <glob> only node hook-admin.mjs ignore-value <rule> "_" # refused: scope it or use ignore-rule
 * node hook-admin.mjs reset # remove all config + cache
 *
 * Designed to be invoked by the LLM from the reference/hooks.md flow. Output is human-readable; the
 * harness will pass it back to the user.
 */
...
const HOOK_MANIFEST_TARGETS = [
    {
        ...
        manifest: () => ({
            description:
                'Impeccable design detector: immediate-tier checks after Edit/Write on UI files, full-rule deep pass on Stop.',
            hooks: {
                PostToolUse: [
                    {
                        matcher: 'Edit|Write',
...
function statusReport(cwd) {
    ...
    const localPath =
        path.relative(cwd, getLocalConfigPath(cwd)) || '.impeccable/config.local.json';
