 ...
 *
 * The browser Save path stages copy edits in .impeccable/live. This script is called by
 * /manual-edit-commit when the user clicks Apply copy edits. It gives the local AI runner the full
 * staged batch plus evidence, validates the files the runner reports touching, and clears only
 * entries reported as applied.
 *
 * Usage: node live-commit-manual-edits.mjs node live-commit-manual-edits.mjs --page-url=/
 *
 * Output JSON: { applied, failed, files, cleared, count, pageUrl }
 */
...
export async function commitManualEdits({
    ...
    const { verifiedIds: verifiedAppliedIds, failed: verificationFailed } =
...
if (process.argv[1]?.endsWith('live-commit-manual-edits.mjs')) {
    main().catch((err) => {
        console.error(
            JSON.stringify({ error: 'commit_failed', message: err.message || String(err) })
        );
        process.exit(1);
    });
}
